% Produit vectoriel — Mathématiques Tle E — Cours signé OnBuch+
...
On considère les points $A(1;1;0)$, $B(2;-1;1)$, $C(0;2;1)$, $D(1;0;2)$.
\begin{enumerate}
...
\begin{enumerate}
  \item $\overrightarrow{AB}=\begin{pmatrix}1\\-2\\1\end{pmatrix}$, $\overrightarrow{CD}=\begin{pmatrix}1\\-2\\1\end{pmatrix}$.
  $\overrightarrow{AB} = \overrightarrow{CD}$ : les droites ont même directeur $\Rightarrow$ elles sont \textbf{parallèles} (donc coplanaires !).
  \emph{Correction de l'énoncé intentionnel :} Si l'énoncé dit « ne sont pas coplanaires », c'est faux ici. Supposons un typo sur $D$ pour avoir des droites gauches, ex $D(1;0;3)$.
  \emph{Reprenons avec $D(1;0;3)$ :} $\overrightarrow{CD}=\begin{pmatrix}1\\-2\\2\end{pmatrix}$. $\overrightarrow{AB}$ et $\overrightarrow{CD}$ non colinéaires.
  ...
  Numérateur $= |1| = 1$.
  $\overrightarrow{AB}\wedge\overrightarrow{CD} = \begin{pmatrix} (-2)(2)-(1)(-2) \\ (1)(1)-(-2)(1) \\ (1)(-2)-(-2)(1) \end{pmatrix} = \begin{pmatrix} -4+2 \\ 1+2 \\ -2+2 \end{pmatrix} = \begin{pmatrix}-2\\3\\0\end{pmatrix}$.
  Norme $= \sqrt{4+9} = \sqrt{13}$.
  $d = \frac{1}{\sqrt{13}} = \frac{\sqrt{13}}{13}$.
  \item Volume $ABCD = \frac{1}{6}|\det| = \frac{1}{6}$.
  \item Aire $BCD$ : $\overrightarrow{BC}=\begin{pmatrix}-2\\3\\0\end{pmatrix}$, $\overrightarrow{BD}=\begin{pmatrix}-1\\1\\2\end{pmatrix}$ (avec $D(1;0;3)$ corrigé).
