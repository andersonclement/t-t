...
\coursec{MA08 : Étude de fonctions}
...
\begin{exoresolu}[Variations d'un polynôme de degré 3]
...
\textbf{Solution détaillée.}
\begin{enumerate}
\item $f'(x) = 3x^2 - 6x - 9 = 3(x^2 - 2x - 3) = 3(x+1)(x-3)$.
$f'(x)=0 \Leftrightarrow x=-1$ ou $x=3$.
Le trinôme $3(x+1)(x-3)$ a le signe de $(x+1)(x-3)$ (coefficient directeur $3>0$).
\begin{center}
...
\end{tabular}
\end{center}
\item D'après le tableau, $f$ admet un maximum local $10$ en $x=-1$ et un minimum local $-22$ en $x=3$.
\item $f(0)=5$, $f'(0)=-9$. L'équation de la tangente au point d'abscisse $0$ est $y = f'(0)(x-0) + f(0)$, soit $y = -9x + 5$.
\item \textbf{Résolution graphique de $f(x)=0$.} On trace la courbe (voir figure). On recherche les intersections avec l'axe des abscisses.
$f(-3)=-22$, $f(-2)=3$ $\Rightarrow$ une racine $\alpha_1 \in [-3,-2]$ (encadrement amplitude $0,5$ : $[-2,5 ; -2]$ ou $[-3 ; -2,5]$).
$f(0)=5$, $f(1)=-6$ $\Rightarrow$ une racine $\alpha_2 \in [0,1]$ (encadrement : $[0,5 ; 1]$).
$f(4)=-15$, $f(5)=10$ $\Rightarrow$ une racine $\alpha_3 \in [4,5]$ (encadrement : $[4,5 ; 5]$).
\end{enumerate}
\begin{popfigure}
\begin{tikzpicture}[scale=0.7, domain=-3.5:5.5]
\draw[->] (-4,0) -- (6,0) node[right] {$x$};
\draw[->] (0,-25) -- (0,12) node[above] {$y$};
\foreach \x in {-3,-2,-1,1,2,3,4,5} \draw (\x,0.15) -- (\x,-0.15) node[below] {\small $\x$};
\foreach \y in {-20,-10,10} \draw (0.15,\y) -- (-0.15,\y) node[left] {\small $\y$};
\draw[popcurve, thick, samples=200] plot (\x, {\x*\x*\x - 3*\x*\x - 9*\x + 5});
\draw[dashed, popmuted] (0,5) node[left] {$5$} -- (0,5);
\draw[dashed, popmuted] (-1,0) -- (-1,10) node[above] {$-1$};
\draw[dashed, popmuted] (3,0) -- (3,-22) node[below] {$3$};
\end{tikzpicture}
\legende{Courbe de $f(x)=x^3-3x^2-9x+5$}
\end{popfigure}
\end{exoresolu}

\begin{exoresolu}[Fonction rationnelle et coût moyen]
...
\textbf{Solution détaillée.}
\begin{enumerate}
\item Division euclidienne ou mise au même dénominateur :
$x + 4 + \dfrac{16}{x} = \dfrac{x^2 + 4x + 16}{x} = CM(x)$.
\item $CM'(x) = 1 - \dfrac{16}{x^2} = \dfrac{x^2 - 16}{x^2} = \dfrac{(x-4)(x+4)}{x^2}$.
Sur $]0\ ; +\infty[$, $x^2 > 0$ et $x+4 > 0$. Le signe de $CM'(x)$ est celui de $x-4$.
$CM'(x) < 0$ sur $]0\ ; 4[$, $CM'(4)=0$, $CM'(x) > 0$ sur $]4\ ; +\infty[$.
...
\end{tabular}
\end{center}
\item Le coût moyen est minimal pour $x=4$ meubles par semaine.
$CM(4) = 12$ dizaines de milliers de FCFA = \textbf{120\,000 FCFA}.
\end{enumerate}
\end{exoresolu}

\begin{exoresolu}[Étude d'une fonction logarithme]
...
\textbf{Solution détaillée.}
\begin{enumerate}
\item $2x+4 > 0 \Leftrightarrow x > -2$. Donc $D_f = ]-2\ ; +\infty[$.
\item $\lim_{x \to -2^+} \ln(2x+4) = -\infty$ et $-x \to 2$, donc $\lim_{x \to -2^+} f(x) = -\infty$.
$\lim_{x \to +\infty} \ln(2x+4) = +\infty$ mais $-x \to -\infty$ domine (croissance logarithmique vs linéaire), donc $\lim_{x \to +\infty} f(x) = -\infty$.
\item $f'(x) = \dfrac{2}{2x+4} - 1 = \dfrac{1}{x+2} - 1 = \dfrac{1 - (x+2)}{x+2} = -\dfrac{x+1}{x+2}$.
Sur $D_f = ]-2\ ; +\infty[$, le dénominateur $x+2 > 0$.
$f'(x) = 0 \Leftrightarrow x = -1$.
$f'(x) > 0$ sur $]-2\ ; -1[$, $f'(x) < 0$ sur $]-1\ ; +\infty[$.
\textbf{Tableau de variations :}
\begin{center}
\begin{tabular}{|c|ccccc|}\hline
$x$ & $-2$ & & $-1$ & & $+\infty$ \\ \hline
$f'(x)$ & & $+$ & $0$ & $-$ & \\ \hline
$f$ & $-\infty$ & $\nearrow$ & $\ln 2 + 1$ & $\searrow$ & $-\infty$ \\ \hline
\end{tabular}
\end{center}
\item $f$ est négative au voisinage de $-2$, passe par un maximum positif $\ln 2 + 1 \approx 1,69 > 0$ en $-1$, puis redescend vers $-\infty$. L'équation $f(x)=0$ admet deux solutions $\alpha \in ]-2\ ; -1[$ et $\beta \in ]-1\ ; +\infty[$.
$f(x) \geq 0$ sur $[\alpha\ ; \beta]$, $f(x) < 0$ sur $]-2\ ; \alpha[ \cup ]\beta\ ; +\infty[$.
\item $\ln(2x+4) < x \Leftrightarrow f(x) < 0$. L'ensemble des solutions est $]-2\ ; \alpha[ \cup ]\beta\ ; +\infty[$.
(On peut approximer $\alpha \approx -1,8$ et $\beta \approx 1,2$ par la calculatrice).
\end{enumerate}
\end{exoresolu}

\begin{exoresolu}[Croissance démographique]
...
\textbf{Solution détaillée.}
\begin{enumerate}
\item $P'(t) = 5 \times 0,03 \,\mathrm{e}^{0,03t} = 0,15\,\mathrm{e}^{0,03t} > 0$ sur $\mathbb{R}$. $P$ est strictement croissante sur $[0\ ; +\infty[$.
\item $P(0) = 5\,\mathrm{e}^0 = 5$ millions d'habitants.
\item $P(T) = 2P(0) \Leftrightarrow 5\,\mathrm{e}^{0,03T} = 10 \Leftrightarrow \mathrm{e}^{0,03T} = 2 \Leftrightarrow 0,03T = \ln 2 \Leftrightarrow T = \dfrac{\ln 2}{0,03}$.
$\ln 2 \approx 0,6931$, donc $T \approx \dfrac{0,6931}{0,03} \approx 23,1$ ans. Arrondi à une année près : \textbf{23 ans}.
\item $P(t) > 10 \Leftrightarrow 5\,\mathrm{e}^{0,03t} > 10 \Leftrightarrow \mathrm{e}^{0,03t} > 2 \Leftrightarrow t > \dfrac{\ln 2}{0,03} \approx 23,1$.
La population dépasse 10 millions pour $t > 23,1$, soit à partir de l'année $2020 + 24 = \textbf{2044}$.
\end{enumerate}
\end{exoresolu}

\courssub{Je me prépare au Bac}

\begin{exoresolu}[Bac type A : étude complète d'une fonction rationnelle]
...
\item Montrer qu'il existe trois réels $a$, $b$ et $c$ tels que, pour tout $x \in D_f$ : $f(x) = ax + b + \dfrac{c}{x - 2}$.
...
\item $\lim_{x \to 2^-} f(x) = \dfrac{1}{0^-} = -\infty$, $\lim_{x \to 2^+} f(x) = \dfrac{1}{0^+} = +\infty$.
La droite d'équation $x=2$ est asymptote verticale à $(\mathcal{C})$.
\item Division euclidienne : $x^2 - 3x + 3 = (x-2)(x-1) + 1$.
Donc $f(x) = x - 1 + \dfrac{1}{x-2}$. On a $a=1$, $b=-1$, $c=1$.
\item $\lim_{x \to \pm\infty} \left(f(x) - (x-1)\right) = \lim_{x \to \pm\infty} \dfrac{1}{x-2} = 0$.
La droite $(\Delta) : y = x-1$ est asymptote oblique en $+\infty$ et $-\infty$.
\item $f(x) - (x-1) = \dfrac{1}{x-2}$.
Pour $x < 2$, $x-2 < 0 \Rightarrow f(x) - (x-1) < 0 \Rightarrow (\mathcal{C})$ est \textbf{sous} $(\Delta)$.
Pour $x > 2$, $x-2 > 0 \Rightarrow f(x) - (x-1) > 0 \Rightarrow (\mathcal{C})$ est \textbf{au-dessus} de $(\Delta)$.
\item $f'(x) = \dfrac{(2x-3)(x-2) - (x^2-3x+3)}{(x-2)^2} = \dfrac{x^2 - 4x + 3}{(x-2)^2} = \dfrac{(x-1)(x-3)}{(x-2)^2}$.
Le dénominateur est $>0$ sur $D_f$. Le numérateur s'annule en $1$ et $3$.
\begin{center}
...
\end{tabular}
\end{center}
\item $f(3) = 3$, $f'(3) = 0$. La tangente $(T)$ en $x=3$ est horizontale : $y = 3$.
\item \textbf{Trace} : Voir figure ci-dessous. Asymptote verticale $x=2$ (pointillés), oblique $y=x-1$ (trait fin), tangente $y=3$ (trait fin), courbe $(\mathcal{C})$ (trait épais).
\item \textbf{Discussion de $f(x)=m$ (intersections avec $y=m$) :}
\begin{itemize}
\item $m < -1$ : 2 solutions (une sur $]-\infty,1[$, une sur $]1,2[$).
\item $m = -1$ : 1 solution ($x=1$, tangente horizontale).
\item $-1 < m < 3$ : 0 solution.
\item $m = 3$ : 1 solution ($x=3$, tangente horizontale).
\item $m > 3$ : 2 solutions (une sur $]2,3[$, une sur $]3,+\infty[$).
\end{itemize}
\end{enumerate}
\begin{popfigure}
...
% Asymptote verticale
\draw[dashed, popmuted] (2,-6) -- (2,6) node[above] {$x=2$};
% Asymptote oblique
\draw[dashed, popblue] (-4,-5) -- (6,5) node[right] {$y=x-1$};
% Tangente
\draw[dashed, poporange] (-4,3) -- (6,3) node[right] {$y=3$};
% Courbe branche gauche
\draw[popcurve, thick, samples=200] plot (\x, {(\x*\x - 3*\x + 3)/(\x - 2)}) node[right] {$(\mathcal{C})$};
% Points remarquables
\fill[popdark] (1,-1) circle (2pt) node[below left] {$(-1)$};
\fill[popdark] (3,3) circle (2pt) node[above right] {$(3,3)$};
\end{tikzpicture}
\legende{Courbe de $f(x)=\frac{x^2-3x+3}{x-2}$, asymptotes et tangente en $3$}
\end{popfigure}
\end{exoresolu}

\begin{exoresolu}[Bac type A : fonction exponentielle et changement de variable]
...
\textbf{Solution détaillée.}
\begin{enumerate}
\item $\lim_{x \to -\infty} \mathrm{e}^x = 0$, donc $\lim_{x \to -\infty} \mathrm{e}^{2x} = 0$.
$\lim_{x \to -\infty} f(x) = 0 - 0 + 1 = 1$.
Graphiquement, la droite $y=1$ est asymptote horizontale à $(\mathcal{C})$ en $-\infty$.
\item $f(x) = \mathrm{e}^{2x} - 2\mathrm{e}^x + 1 = \mathrm{e}^x(\mathrm{e}^x - 2) + 1$.
$\lim_{x \to +\infty} \mathrm{e}^x = +\infty$, donc $\lim_{x \to +\infty} f(x) = +\infty$.
\item $f'(x) = 2\mathrm{e}^{2x} - 2\mathrm{e}^x = 2\mathrm{e}^x(\mathrm{e}^x - 1)$.
\item $\mathrm{e}^x > 0$ toujours. $f'(x)$ a le signe de $\mathrm{e}^x - 1$.
$f'(x) = 0 \Leftrightarrow \mathrm{e}^x = 1 \Leftrightarrow x = 0$.
$f'(x) < 0$ sur $]-\infty\ ; 0[$, $f'(x) > 0$ sur $]0\ ; +\infty[$.
\textbf{Tableau de variations :}
\begin{center}
\begin{tabular}{|c|ccccc|}\hline
...
\end{tabular}
\end{center}
\item Posons $X = \mathrm{e}^x > 0$. $f(x)=0 \Leftrightarrow X^2 - 2X + 1 = 0 \Leftrightarrow (X-1)^2 = 0 \Leftrightarrow X = 1$.
$\mathrm{e}^x = 1 \Leftrightarrow x = 0$. Solution unique : $x=0$.
\item D'après le tableau, le minimum de $f$ est $f(0)=0$. Donc $f(x) \geq 0$ pour tout $x \in \mathbb{R}$.
On retrouve $f(x) = (\mathrm{e}^x - 1)^2 \geq 0$.
\item Points : $A(-2\ ; f(-2)\approx 0,75)$, $B(-1\ ; f(-1)\approx 0,40)$, $C(0\ ; 0)$, $D(1\ ; f(1)\approx 2,95)$.
\textbf{Trace} : Voir figure.
\item $f(x) < 1 \Leftrightarrow \mathrm{e}^{2x} - 2\mathrm{e}^x + 1 < 1 \Leftrightarrow \mathrm{e}^x(\mathrm{e}^x - 2) < 0$.
Comme $\mathrm{e}^x > 0$, cela revient à $\mathrm{e}^x - 2 < 0 \Leftrightarrow \mathrm{e}^x < 2 \Leftrightarrow x < \ln 2$.
Ensemble des solutions : $]-\infty\ ; \ln 2[$.
\end{enumerate}
\begin{popfigure}
\begin{tikzpicture}[scale=1.2, domain=-2.5:1.5]
...
\draw[dashed, popmuted] (-3,1) -- (2,1) node[right] {$y=1$};
\draw[popcurve, thick, samples=200] plot (\x, {exp(2*\x) - 2*exp(\x) + 1});
\fill[popdark] (-2,0.748) circle (1.5pt) node[above left] {$A$};
\fill[popdark] (-1,0.399) circle (1.5pt) node[above left] {$B$};
\fill[popdark] (0,0) circle (1.5pt) node[below left] {$C(0,0)$};
\fill[popdark] (1,2.952) circle (1.5pt) node[above right] {$D$};
\end{tikzpicture}
\legende{Courbe de $f(x)=\mathrm{e}^{2x}-2\mathrm{e}^x+1$}
\end{popfigure}
\end{exoresolu}

\begin{exoresolu}[Bac type A : problème d'optimisation en entreprise]
...
\textbf{Solution détaillée.}
\begin{enumerate}
\item $B(0) = 0$. Si l'entreprise ne produit rien, le bénéfice est nul (pas de chiffre d'affaires, pas de coûts variables).
$B(10) = -1000 + 1200 - 360 = -160$ millions de FCFA. Une production de 10 tonnes entraîne une perte de 160 millions de FCFA.
\item $B'(x) = -3x^2 + 24x - 36 = -3(x^2 - 8x + 12) = -3(x-2)(x-6)$.
\item Sur $[0\ ; 10]$, $-3 < 0$.
$x-2$ : négatif sur $[0\ ; 2[$, nul en $2$, positif sur $]2\ ; 10]$.
$x-6$ : négatif sur $[0\ ; 6[$, nul en $6$, positif sur $]6\ ; 10]$.
Produit $(x-2)(x-6)$ : $+$ sur $[0\ ; 2[$, $-$ sur $]2\ ; 6[$, $+$ sur $]6\ ; 10]$.
$B'(x) = -3 \times \text{signe}((x-2)(x-6))$ : $-$ sur $[0\ ; 2[$, $+$ sur $]2\ ; 6[$, $-$ sur $]6\ ; 10]$.
\textbf{Tableau de variations :}
\begin{center}
\begin{tabular}{|c|ccccccc|}\hline
$x$ & $0$ & & $2$ & & $6$ & & $10$ \\ \hline
...
\end{tabular}
\end{center}
\item $B$ décroît sur $[0\ ; 2]$, croît sur $[2\ ; 6]$, décroît sur $[6\ ; 10]$.
Les maximums sur le segment sont aux bornes ou aux extrema locaux : $B(0)=0$, $B(6)=0$, $B(10)=-160$.
Le bénéfice maximal est \textbf{0 million de FCFA} (soit 0 FCFA), atteint pour $x=0$ (arrêt de production) et $x=6$ tonnes.
Si l'entreprise doit produire ($x>0$), la production optimale est \textbf{6 tonnes} pour un bénéfice nul.
\item $B(x) = -x^3 + 12x^2 - 36x = -x(x^2 - 12x + 36) = -x(x-6)^2$.
Sur $[0\ ; 10]$, $x \geq 0$ et $(x-6)^2 \geq 0$, donc $-x(x-6)^2 \leq 0$.
$B(x) > 0$ n'a \textbf{aucune solution} sur $[0\ ; 10]$. L'entreprise n'est jamais bénéficiaire (au mieux à l'équilibre pour $x=0$ ou $x=6$).
\item Le directeur a \textbf{tort}.
D'après le tableau, le minimum global (pire bénéfice) sur $[0\ ; 10]$ est $B(10) = -160$ millions (perte maximale).
En $x=6$, $B(6)=0$ est un \textbf{maximum local} (et global sur l'intervalle). Produire 6 tonnes assure l'équilibre, ce qui est la meilleure situation possible (avec l'arrêt de production), pas la pire.
\end{enumerate}
\end{exoresolu}

\courssub{Synthèse : Méthode générale d'étude et comparaison des croissances}

\begin{methode}[Étude complète d'une fonction (fiche Bac)]
Pour toute fonction $f$ dérivable sur son ensemble de définition $D_f$ :
\begin{enumerate}
\item \textbf{Ensemble de définition $D_f$} : identifier les valeurs interdites (dénominateur nul, argument du $\ln \le 0$, etc.).
\item \textbf{Parité / Périodicité} (si applicable) : simplifie l'étude.
\item \textbf{Limites aux bornes de $D_f$} : déterminer les asymptotes verticales (limites infinies en $x_0$ fini) et horizontales/obliques (limites finies ou infinies en $\pm\infty$).
\item \textbf{Dérivée $f'(x)$} : calculer, factoriser si possible.
\item \textbf{Tableau de signes de $f'$} $\rightarrow$ \textbf{Tableau de variations de $f$} (valeurs exactes aux points remarquables, limites aux bornes).
\item \textbf{Équation de la tangente} en un point $a \in D_f$ : $y = f'(a)(x-a) + f(a)$.
\item \textbf{Position relative par rapport aux asymptotes} : signe de $f(x) - (ax+b)$.
\item \textbf{Tracé de la courbe} : repère orthonormé, unités adaptées, points caractéristiques, asymptotes (trait pointillé), tangentes.
\item \textbf{Résolution d'équations / inéquations} : $f(x)=m$ (intersections avec $y=m$), $f(x)<0$ (signe de $f$).
\end{enumerate}
\end{methode}

\begin{aretenir}[Asymptotes]
\begin{itemize}
\item \textbf{Verticale} $x = a$ : $\lim_{x \to a^\pm} f(x) = \pm\infty$ ($a$ borne de $D_f$).
\item \textbf{Horizontale} $y = \ell$ : $\lim_{x \to \pm\infty} f(x) = \ell$ (fini).
\item \textbf{Oblique} $y = ax + b$ ($a \neq 0$) en $+\infty$ (ou $-\infty$) :
$a = \lim_{x \to +\infty} \frac{f(x)}{x}$, $b = \lim_{x \to +\infty} (f(x) - ax)$.
\emph{Astuce pour les fractions rationnelles} : faire la division euclidienne du numérateur par le dénominateur.
\end{itemize}
\end{aretenir}

\begin{aretenir}[Croissances comparées (références pour les limites)]
\begin{align*}
&\text{Pour } x \to +\infty : & x^\alpha &\ll \mathrm{e}^x \quad (\alpha>0) & \ln x &\ll x^\alpha \quad (\alpha>0) \\
&\text{Pour } x \to 0^+ : & \ln x &\to -\infty & x^\alpha \ln x &\to 0 \quad (\alpha>0)
\end{align*}
\cle{Règle} : L'exponentielle l'emporte sur toute puissance ; toute puissance l'emporte sur le logarithme.
\end{aretenir}

\begin{attention}[Pièges fréquents au Bac]
\begin{itemize}
\item \textbf{Domaine du $\ln$} : $\ln(u(x))$ impose $u(x) > 0$ (strict), pas $\geq 0$.
\item \textbf{Asymptote oblique} : Vérifier \textbf{les deux bornes} $+\infty$ et $-\infty$ si $D_f$ n'est pas borné des deux côtés. Les coefficients $a,b$ peuvent différer.
\item \textbf{Tableau de variations} : Ne pas oublier la double barre $||$ pour les valeurs interdites (asymptotes verticales). Les flèches ($\nearrow, \searrow$) vont \textbf{uniquement} sur la ligne de $f$, les signes sur la ligne de $f'$.
\item \textbf{Factorisation} : Toujours factoriser $f'(x)$ pour en étudier le signe (forme $a(x-x_1)(x-x_2)$ ou $\frac{N(x)}{D(x)}$).
\item \textbf{Équation $f(x)=m$} : Lire les intersections sur le graphique ou utiliser le tableau de variations (valeurs intermédiaires).
\item \textbf{Unités} : Convertir les unités du problème (dizaines de milliers, millions, tonnes) dans la réponse finale.
\end{itemize}
\end{attention}

\begin{savaistu}[Le nombre $e$ et le Cameroun]
Le nombre $e \approx 2,718$ est la base des logarithmes népériens. Il apparaît naturellement dans les phénomènes de \textbf{croissance continue} : démographie (comme dans l'exercice sur la population), finance (intérêts composés continus), radioactivité, épidémiologie. Au Cameroun, la modélisation de la croissance urbaine (Yaoundé, Douala) ou de la propagation d'une maladie (choléra, COVID-19) utilise souvent des fonctions exponentielles de base $e$. La fonction $\ln$ est son inverse : elle permet de \textbf{remonter le temps} (calculer le temps de doublement, la demi-vie).
\end{savaistu}
...
\coursec{Exercices corrigés et approfondis — Étude de fonctions}

\begin{corrige}[Exercice 1 — QCM : Dérivées, limites et ensembles de définition]
\begin{enumerate}
    \item Soit la fonction $f$ définie sur $\mathbb{R}$ par $f(x) = x^3 - 3x^2 - 9x + 5$. Sa dérivée est :
    \begin{itemize}
        \item \textbf{a.} $f'(x) = 3x^2 - 6x - 9$
        \item b. $f'(x) = 3x^2 - 3x - 9$
        \item c. $f'(x) = 3x^2 - 6x + 9$
    \end{itemize}
    \textbf{Réponse : a.} $f$ est un polynôme, dérivable sur $\mathbb{R}$. On dérive terme à terme :
    \[ f'(x) = 3x^2 - 2\times 3x - 9 = 3x^2 - 6x - 9. \]

    \item Soit la fonction $g$ définie par $g(x) = \ln(2x+4)$. Son ensemble de définition est :
    \begin{itemize}
        \item a. $[-2\ ; +\infty[$
        \item \textbf{b.} $]-2\ ; +\infty[$
        \item c. $]-\infty\ ; -2[$
    \end{itemize}
    \textbf{Réponse : b.} La fonction logarithme népérien exige un argument \emph{strictement} positif :
    \[ 2x + 4 > 0 \iff 2x > -4 \iff x > -2. \]
    Donc $D_g = ]-2\ ; +\infty[$. La borne $-2$ est exclue (parenthèse).

    \item On considère la limite $\displaystyle \lim_{x \to -\infty} \mathrm{e}^{2x}$. La valeur de cette limite est :
    \begin{itemize}
        \item a. $+\infty$
        \item \textbf{b.} $0$
        \item c. $1$
    \end{itemize}
    \textbf{Réponse : b.} Posons $X = 2x$. Quand $x \to -\infty$, $X \to -\infty$. Or $\lim_{X \to -\infty} \mathrm{e}^X = 0$. Par composition, $\lim_{x \to -\infty} \mathrm{e}^{2x} = 0$.

    \item Soit la fonction $h$ définie par $h(x) = \dfrac{x^2 - 3x + 3}{x - 2}$. La droite d'équation $x = 2$ est-elle une asymptote verticale ?
    \begin{itemize}
        \item \textbf{a.} Oui
        \item b. Non
    \end{itemize}
    \textbf{Réponse : a.} Le dénominateur s'annule en $x = 2$. Le numérateur vaut $2^2 - 3\times 2 + 3 = 1 \neq 0$. Donc $\lim_{x \to 2} h(x) = \pm\infty$ (selon le côté), ce qui caractérise une asymptote verticale d'équation $x = 2$. \textbf{Rappel :} une asymptote verticale a toujours une équation de la forme $x = a$, jamais $y = a$.
\end{enumerate}
\end{corrige}

\begin{corrige}[Exercice 2 — Vrai ou Faux ? Justifier soigneusement]
Soit la fonction $f$ définie sur $\mathbb{R}$ par $f(x) = \left(\mathrm{e}^x - 1\right)^2 = \mathrm{e}^{2x} - 2\mathrm{e}^x + 1$.

\begin{enumerate}
    \item \textbf{Vrai.} En développant l'identité remarquable $(a-b)^2 = a^2 - 2ab + b^2$ avec $a = \mathrm{e}^x$ et $b = 1$ :
    \[ \left(\mathrm{e}^x - 1\right)^2 = \left(\mathrm{e}^x\right)^2 - 2\mathrm{e}^x + 1 = \mathrm{e}^{2x} - 2\mathrm{e}^x + 1 = f(x), \]
    car $\left(\mathrm{e}^x\right)^2 = \mathrm{e}^{2x}$ (règle des puissances : $(a^m)^n = a^{mn}$).

    \item \textbf{Faux.} La dérivée de $f$ sur $\mathbb{R}$ est :
    \[ f'(x) = (\mathrm{e}^{2x})' - (2\mathrm{e}^x)' + (1)' = 2\mathrm{e}^{2x} - 2\mathrm{e}^x. \]
    L'erreur fréquente est d'écrire $f'(x) = 2\mathrm{e}^{2x} - 2$ (oubli du facteur $\mathrm{e}^x$ dans la dérivée de $-2\mathrm{e}^x$). Rappel : $(\mathrm{e}^{u(x)})' = u'(x)\mathrm{e}^{u(x)}$.

    \item \textbf{Vrai.} Étudions la limite en $+\infty$. Factorisons par le terme dominant $\mathrm{e}^{2x}$ :
    \[ f(x) = \mathrm{e}^{2x}\left(1 - 2\mathrm{e}^{-x} + \mathrm{e}^{-2x}\right). \]
    Quand $x \to +\infty$, $\mathrm{e}^{-x} \to 0$ et $\mathrm{e}^{-2x} \to 0$, donc la parenthèse tend vers $1$. Par produit, $\mathrm{e}^{2x} \to +\infty$ et $1 \to 1$, donc $\lim_{x \to +\infty} f(x) = +\infty$.

    \item \textbf{Vrai.} Résolvons $f(x) = 0$ :
    \[ f(x) = 0 \iff \left(\mathrm{e}^x - 1\right)^2 = 0 \iff \mathrm{e}^x - 1 = 0 \iff \mathrm{e}^x = 1 \iff x = 0. \]
    L'équation admet une unique solution : $x = 0$ (racine double).
\end{enumerate}
\end{corrige}

\begin{aretenir}[Rappels de cours indispensables pour le Bac]
\textbf{Asymptote verticale.} La droite d'équation $x = a$ est une asymptote verticale à la courbe $\mathcal{C}_f$ de la fonction $f$ si et seulement si au moins l'une des limites latérales est infinie :
\[ \lim_{x \to a^-} f(x) = \pm\infty \quad \text{ou} \quad \lim_{x \to a^+} f(x) = \pm\infty. \]

\textbf{Asymptote oblique.} La droite d'équation $y = ax + b$ (avec $a \neq 0$) est une asymptote oblique à $\mathcal{C}_f$ en $+\infty$ (resp. $-\infty$) si et seulement si :
\[ \lim_{x \to +\infty} \bigl(f(x) - (ax + b)\bigr) = 0 \quad \left(\text{resp. } \lim_{x \to -\infty} \bigl(f(x) - (ax + b)\bigr) = 0\right). \]
\textit{Méthode pratique :} $a = \lim_{x \to \pm\infty} \frac{f(x)}{x}$ et $b = \lim_{x \to \pm\infty} \bigl(f(x) - ax\bigr)$.

\textbf{Tangente.} Si $f$ est dérivable en $a$, la tangente à $\mathcal{C}_f$ au point d'abscisse $a$ a pour équation :
\[ y = f'(a)(x - a) + f(a). \]

\textbf{Limites de référence (croissances comparées).}
\[ \lim_{x \to 0^+} \ln x = -\infty, \qquad \lim_{x \to +\infty} \ln x = +\infty, \qquad \lim_{x \to +\infty} \frac{\ln x}{x} = 0. \]
\[ \lim_{x \to -\infty} \mathrm{e}^x = 0, \qquad \lim_{x \to +\infty} \mathrm{e}^x = +\infty, \qquad \lim_{x \to +\infty} \frac{\mathrm{e}^x}{x^n} = +\infty \ (\forall n \in \mathbb{N}). \]
\end{aretenir}

\begin{corrige}[Exercice 4 — Calculs directs : domaines, dérivées, limites]
\begin{enumerate}
    \item \textbf{Ensembles de définition.}
    \begin{itemize}
        \item $f(x) = \dfrac{x^2+1}{x-3}$. Le dénominateur s'annule pour $x=3$. $D_f = \mathbb{R} \setminus \{3\} = ]-\infty\ ; 3[ \cup ]3\ ; +\infty[$.
        \item $g(x) = \ln(3x - 6)$. Condition : $3x - 6 > 0 \iff x > 2$. $D_g = ]2\ ; +\infty[$.
        \item $h(x) = \mathrm{e}^{-2x+1}$. L'exponentielle est définie sur $\mathbb{R}$. $D_h = \mathbb{R}$.
    \end{itemize}

    \item \textbf{Dérivées (sur les ensembles de définition respectifs).}
    \begin{itemize}
        \item $f = \dfrac{u}{v}$ avec $u(x)=x^2+1$, $u'(x)=2x$, $v(x)=x-3$, $v'(x)=1$ :
        \[ f'(x) = \frac{u'v - uv'}{v^2} = \frac{2x(x-3) - (x^2+1)}{(x-3)^2} = \frac{x^2 - 6x - 1}{(x-3)^2}, \quad x \neq 3. \]
        \item $g = \ln \circ u$ avec $u(x)=3x-6$, $u'(x)=3$ :
        \[ g'(x) = \frac{u'(x)}{u(x)} = \frac{3}{3x-6} = \frac{1}{x-2}, \quad x > 2. \]
        \textit{Attention :} bien que l'expression $\frac{1}{x-2}$ soit définie pour $x \neq 2$, la dérivée $g'$ n'existe que sur $D_g = ]2\ ; +\infty[$.
        \item $h = \mathrm{e}^{u}$ avec $u(x)=-2x+1$, $u'(x)=-2$ :
        \[ h'(x) = u'(x)\mathrm{e}^{u(x)} = -2\,\mathrm{e}^{-2x+1}, \quad x \in \mathbb{R}. \]
    \end{itemize}

    \item \textbf{Limites du polynôme $P(x) = x^3 - 3x^2 - 9x + 5$ aux bornes de $\mathbb{R}$.}
    Le terme de plus haut degré est $x^3$. Pour un polynôme, la limite en $\pm\infty$ est la limite de son terme dominant :
    \[ \lim_{x \to +\infty} P(x) = \lim_{x \to +\infty} x^3 = +\infty, \qquad \lim_{x \to -\infty} P(x) = \lim_{x \to -\infty} x^3 = -\infty. \]
\end{enumerate}
\end{corrige}

\courssub{Études complètes et problèmes de modélisation (Cœur du programme)}

\begin{exoresolu}[Exercice 5 — Étude complète : fonction rationnelle]
\textbf{Énoncé.} Soit la fonction $f$ définie par $f(x) = \dfrac{2x^2 - 5x + 2}{x - 1}$.
\begin{enumerate}
    \item Déterminer $D_f$ et étudier la parité/parité.
    \item Calculer les limites aux bornes de $D_f$ et rechercher les asymptotes.
    \item Calculer $f'(x)$, dresser le tableau de variations de $f$.
    \item Tracer la courbe $\mathcal{C}_f$ dans un repère orthonormé.
\end{enumerate}
\tcblower
\textbf{Solution détaillée.}

\textbf{1. Ensemble de définition et symétries.}
$D_f = \mathbb{R} \setminus \{1\} = ]-\infty\ ; 1[ \cup ]1\ ; +\infty[$.
$f(-x) = \frac{2x^2 + 5x + 2}{-x-1} \neq f(x)$ et $\neq -f(x)$ : pas de symétrie par rapport à l'axe des ordonnées ni par rapport à l'origine.

\textbf{2. Limites et asymptotes.}
\begin{itemize}
    \item \textit{Au voisinage de $1$ (valeur interdite) :}
    Numérateur en $1$ : $2 - 5 + 2 = -1 \neq 0$.
    $\lim_{x \to 1^-} f(x) = \frac{-1}{0^-} = +\infty$, $\lim_{x \to 1^+} f(x) = \frac{-1}{0^+} = -\infty$.
    \cle{Asymptote verticale} : $\mathcal{V}$ d'équation $x = 1$.
    \item \textit{En $\pm\infty$ (degrés : num=2, den=1) :} Asymptote oblique.
    Division euclidienne : $(2x^2 - 5x + 2) \div (x-1) = 2x - 3$ reste $-1$.
    \[ f(x) = 2x - 3 - \frac{1}{x-1}. \]
    $\lim_{x \to \pm\infty} \frac{1}{x-1} = 0$, donc $\lim_{x \to \pm\infty} (f(x) - (2x-3)) = 0$.
    \cle{Asymptote oblique} : $\mathcal{D}$ d'équation $y = 2x - 3$ (en $+\infty$ et $-\infty$).
    \item \textit{Position relative par rapport à $\mathcal{D}$ :} $f(x) - (2x-3) = -\frac{1}{x-1}$.
    Pour $x < 1$, $x-1 < 0 \Rightarrow -\frac{1}{x-1} > 0$ : $\mathcal{C}_f$ au-\cle{dessus} de $\mathcal{D}$.
    Pour $x > 1$, $x-1 > 0 \Rightarrow -\frac{1}{x-1} < 0$ : $\mathcal{C}_f$ en-\cle{dessous} de $\mathcal{D}$.
\end{itemize}

\textbf{3. Dérivée et variations.}
$f(x) = 2x - 3 - (x-1)^{-1}$.
$f'(x) = 2 + (x-1)^{-2} = 2 + \frac{1}{(x-1)^2} = \frac{2(x-1)^2 + 1}{(x-1)^2} = \frac{2x^2 - 4x + 3}{(x-1)^2}$.
Numérateur : $\Delta = 16 - 24 = -8 < 0$, coefficient directeur $2 > 0 \Rightarrow$ numérateur $> 0$ sur $\mathbb{R}$.
Dénominateur $(x-1)^2 > 0$ sur $D_f$.
Donc $f'(x) > 0$ sur $]-\infty\ ; 1[$ et sur $]1\ ; +\infty[$.
$f$ est \cle{strictement croissante} sur chacun de ces deux intervalles.
...
\end{tabular}
\end{center}
\textit{Note :} La double colonne pour $x=1$ marque la coupure de l'ensemble de définition.

\textbf{4. Points remarquables pour le tracé.}
\begin{itemize}
    \item Intersection avec l'axe des abscisses : $f(x)=0 \iff 2x^2-5x+2=0 \iff (2x-1)(x-2)=0 \iff x=\frac{1}{2}$ ou $x=2$. Points $A(\frac{1}{2},0)$ et $B(2,0)$.
    \item Intersection avec l'axe des ordonnées : $f(0) = \frac{2}{-1} = -2$. Point $C(0,-2)$.
    \item Point d'intersection avec l'asymptote oblique ? $f(x) = 2x-3 \iff -\frac{1}{x-1}=0$, impossible. Pas d'intersection.
\end{itemize}
...
\begin{axis}[
    ...
    xtick={-3,-2,-1,0,1,2,3,4}, ytick={-6,-4,-2,0,2,4,6},
    grid=major, grid style={popline}, tick label style={font=\small},
    clip=false
]
% Asymptotes
\addplot[popmuted, dashed, domain=-4:5, samples=2] {2*x-3} node[above right, pos=0.9, font=\tiny, popmuted] {$y=2x-3$};
\draw[popmuted, dashed] (1,-8) -- (1,8) node[above, font=\tiny, popmuted] {$x=1$};
% Courbe branche gauche
\addplot[popcurve, domain=-3.5:0.9, samples=200] { (2*x^2-5*x+2)/(x-1) };
% Courbe branche droite
\addplot[popcurve, domain=1.1:4.5, samples=200] { (2*x^2-5*x+2)/(x-1) };
% Points
\addplot[only marks, mark=*, popblue] coordinates {(0.5,0) (2,0) (0,-2)};
\node[below left, font=\tiny] at (axis cs:0.5,0) {$A$};
\node[above right, font=\tiny] at (axis cs:2,0) {$B$};
\node[below left, font=\tiny] at (axis cs:0,-2) {$C$};
\end{axis}
\end{tikzpicture}
\legende{Courbe $\mathcal{C}_f$ de la fonction rationnelle $f(x) = \frac{2x^2-5x+2}{x-1}$ avec ses asymptotes.}
\end{popfigure}
\end{exoresolu}

\begin{exoresolu}[Exercice 6 — Étude complète : fonction logarithmique]
\textbf{Énoncé.} Soit la fonction $g$ définie par $g(x) = \ln(3x - 2) - 2x$.
\begin{enumerate}
    \item Déterminer $D_g$ et calculer les limites aux bornes.
    \item Calculer $g'(x)$, étudier son signe et dresser le tableau de variations.
    \item Montrer que l'équation $g(x) = 0$ admet une unique solution $\alpha$ sur $D_g$. Donner un encadrement de $\alpha$ au $10^{-1}$ près.
    \item Tracer la courbe $\mathcal{C}_g$.
\end{enumerate}
\tcblower
\textbf{Solution détaillée.}

\textbf{1. Domaine et limites.}
$D_g = \{x \in \mathbb{R} \mid 3x - 2 > 0\} = ]\frac{2}{3}\ ; +\infty[$.
\begin{itemize}
    \item $x \to (\frac{2}{3})^+$ : $3x-2 \to 0^+ \Rightarrow \ln(3x-2) \to -\infty$, et $-2x \to -\frac{4}{3}$. Donc $\lim_{x \to (2/3)^+} g(x) = -\infty$.
    \cle{Asymptote verticale} : $x = \frac{2}{3}$.
    \item $x \to +\infty$ : $\ln(3x-2) \sim \ln x$, mais $-2x$ domine ($\lim_{+\infty} \frac{\ln x}{x} = 0$).
    $g(x) = x\left(\frac{\ln(3x-2)}{x} - 2\right) \to +\infty \times (0 - 2) = -\infty$.
    $\lim_{x \to +\infty} g(x) = -\infty$.
\end{itemize}

\textbf{2. Dérivée et variations.}
$g'(x) = \frac{3}{3x-2} - 2 = \frac{3 - 2(3x-2)}{3x-2} = \frac{3 - 6x + 4}{3x-2} = \frac{7 - 6x}{3x-2}$.
Sur $D_g = ]\frac{2}{3}\ ; +\infty[$, le dénominateur $3x-2 > 0$.
Signe de $g'$ = signe du numérateur $7 - 6x$.
$7 - 6x = 0 \iff x = \frac{7}{6} \in D_g$.
\begin{itemize}
    \item Sur $]\frac{2}{3}\ ; \frac{7}{6}]$ : $7-6x \ge 0 \Rightarrow g'(x) \ge 0$ : $g$ \cle{croissante}.
    \item Sur $[\frac{7}{6}\ ; +\infty[$ : $7-6x \le 0 \Rightarrow g'(x) \le 0$ : $g$ \cle{décroissante}.
\end{itemize}
Maximum en $x = \frac{7}{6}$ : $g(\frac{7}{6}) = \ln(3\times\frac{7}{6}-2) - 2\times\frac{7}{6} = \ln(\frac{3}{2}) - \frac{7}{3} \approx 0,405 - 2,333 = -1,928 < 0$.

\begin{center}
\begin{tabular}{|c|ccccc|}\hline
$x$ & $\frac{2}{3}$ & & $\frac{7}{6}$ & & $+\infty$ \\ \hline
$g'(x)$ & $+\infty$ & $+$ & $0$ & $-$ & $-\infty$ \\ \hline
$g$ & $-\infty$ & $\nearrow$ & $-1,93$ & $\searrow$ & $-\infty$ \\ \hline
\end{tabular}
\end{center}

\textbf{3. Équation $g(x)=0$.}
Sur $D_g$, $g$ est continue, strictement croissante sur $]\frac{2}{3}\ ; \frac{7}{6}]$ puis strictement décroissante sur $[\frac{7}{6}\ ; +\infty[$.
Son maximum est $g(\frac{7}{6}) \approx -1,93 < 0$.
Donc $g(x) < 0$ pour tout $x \in D_g$. L'équation $g(x)=0$ n'admet \textbf{aucune solution} sur $D_g$.
\textit{Attention :} Vérifier le signe du maximum est crucial ! Ici, pas de solution.

\textbf{4. Tracé.}
Courbe entièrement sous l'axe des abscisses, partant de $-\infty$ à la VA $x=2/3$, montant jusqu'au max en $(1,17\ ; -1,93)$, puis redescendant vers $-\infty$.

\begin{popfigure}
\begin{tikzpicture}[scale=0.8]
\begin{axis}[
    axis lines=middle, xlabel=$x$, ylabel=$y$,
    xmin=0.5, xmax=4, ymin=-6, ymax=1,
    xtick={0.66,1,1.16,2,3}, xticklabels={$\frac{2}{3}$,$1$,$\frac{7}{6}$,$2$,$3$},
    ytick={-5,-4,-3,-2,-1,0}, grid=major, grid style={popline},
    clip=false
]
\draw[popmuted, dashed] (0.666,-6) -- (0.666,1) node[above, font=\tiny, popmuted] {$x=\frac{2}{3}$};
\addplot[popcurve, domain=0.67:3.8, samples=300] { ln(3*x-2) - 2*x };
\addplot[only marks, mark=*, poporange] coordinates {(1.1666,-1.928)};
\node[above right, font=\tiny] at (axis cs:1.1666,-1.928) {$M$};
\end{axis}
\end{tikzpicture}
\legende{Courbe $\mathcal{C}_g$ de $g(x)=\ln(3x-2)-2x$.}
\end{popfigure}
\end{exoresolu}

\begin{exoresolu}[Exercice 7 — Étude complète : fonction exponentielle]
\textbf{Énoncé.} Soit la fonction $h$ définie sur $\mathbb{R}$ par $h(x) = (2x - 1)\mathrm{e}^{-x}$.
\begin{enumerate}
    \item Calculer les limites en $\pm\infty$. Rechercher les asymptotes.
    \item Calculer $h'(x)$, dresser le tableau de variations.
    \item Déterminer l'équation de la tangente $\mathcal{T}$ à $\mathcal{C}_h$ au point d'abscisse $0$.
    \item Tracer $\mathcal{C}_h$ et $\mathcal{T}$.
\end{enumerate}
\tcblower
\textbf{Solution détaillée.}
...
$D_h = \mathbb{R}$.
\begin{itemize}
    \item $x \to +\infty$ : $\mathrm{e}^{-x} \to 0$ exponentiellement, $2x-1 \to +\infty$ polynomialement. L'exponentielle l'emporte : $h(x) \to 0$.
    Plus précisément $h(x) > 0$ pour $x$ grand $\Rightarrow \lim_{+\infty} h(x) = 0^+$.
    \cle{Asymptote horizontale} : $y = 0$ (axe des abscisses) en $+\infty$.
    \item $x \to -\infty$ : $\mathrm{e}^{-x} = \mathrm{e}^{|x|} \to +\infty$, $2x-1 \to -\infty$. Produit $\to -\infty$.
    $\lim_{x \to -\infty} h(x) = -\infty$. Pas d'asymptote oblique (croissance exponentielle).
\end{itemize}

\textbf{2. Dérivée et variations.}
$h(x) = u(x)v(x)$ avec $u(x)=2x-1$, $u'=2$ ; $v(x)=\mathrm{e}^{-x}$, $v'=-\mathrm{e}^{-x}$.
$h'(x) = 2\mathrm{e}^{-x} - (2x-1)\mathrm{e}^{-x} = \mathrm{e}^{-x}(2 - 2x + 1) = \mathrm{e}^{-x}(3 - 2x)$.
$\mathrm{e}^{-x} > 0$ sur $\mathbb{R}$. Signe de $h'$ = signe de $3-2x$.
$3-2x = 0 \iff x = \frac{3}{2}$.
\begin{itemize}
    \item $]-\infty\ ; \frac{3}{2}]$ : $3-2x \ge 0 \Rightarrow h' \ge 0$ : $h$ \cle{croissante}.
    \item $[\frac{3}{2}\ ; +\infty[$ : $3-2x \le 0 \Rightarrow h' \le 0$ : $h$ \cle{décroissante}.
\end{itemize}
Maximum global en $x=\frac{3}{2}$ : $h(\frac{3}{2}) = (3-1)\mathrm{e}^{-3/2} = 2\mathrm{e}^{-1.5} \approx 0,446$.

\begin{center}
\begin{tabular}{|c|ccccc|}\hline
$x$ & $-\infty$ & & $\frac{3}{2}$ & & $+\infty$ \\ \hline
$h'(x)$ & & $+$ & $0$ & $-$ & \\ \hline
$h$ & $-\infty$ & $\nearrow$ & $2\mathrm{e}^{-3/2}$ & $\searrow$ & $0^+$ \\ \hline
\end{tabular}
\end{center}

\textbf{3. Tangente en $0$.}
$h(0) = (0-1)\mathrm{e}^0 = -1$.
$h'(0) = \mathrm{e}^0(3-0) = 3$.
Équation : $y = 3(x - 0) - 1 \iff y = 3x - 1$.

\textbf{4. Points remarquables.}
$h(x)=0 \iff 2x-1=0 \iff x=0,5$. Point $A(0,5\ ; 0)$.
$h(0)=-1$. Point $B(0,-1)$ (point de contact de la tangente).

\begin{popfigure}
\begin{tikzpicture}[scale=0.8]
\begin{axis}[
    axis lines=middle, xlabel=$x$, ylabel=$y$,
    xmin=-2, xmax=5, ymin=-3, ymax=1.5,
    xtick={-1,0,0.5,1.5,2,3,4}, ytick={-2,-1,0,0.5,1},
    grid=major, grid style={popline}, clip=false
]
% Asymptote horizontale
\addplot[popmuted, dashed, domain=-2:5] {0};
% Tangente
\addplot[popgreen, dashed, domain=-1:1] {3*x-1} node[above right, pos=0.9, font=\tiny, popgreen] {$\mathcal{T}: y=3x-1$};
% Courbe
\addplot[popcurve, domain=-1.8:4.5, samples=300] { (2*x-1)*exp(-x) };
% Points
\addplot[only marks, mark=*, popblue] coordinates {(0.5,0) (0,-1) (1.5,0.446)};
\node[below right, font=\tiny] at (axis cs:0.5,0) {$A$};
\node[below left, font=\tiny] at (axis cs:0,-1) {$B$};
\node[above, font=\tiny] at (axis cs:1.5,0.446) {$M$};
\end{axis}
\end{tikzpicture}
\legende{Courbe $\mathcal{C}_h$ de $h(x)=(2x-1)\mathrm{e}^{-x}$, tangente en $0$ et asymptote horizontale.}
\end{popfigure}
\end{exoresolu}

\begin{exoresolu}[Exercice 8 — Problème de modélisation : Optimisation du coût moyen]
\textbf{Énoncé.} Une usine de transformation de cacao à Douala produit $x$ tonnes de fèves par mois ($x > 0$). Le coût total de production (en millions de FCFA) est modélisé par la fonction $C(x) = x^2 + 4x + 100$.
\begin{enumerate}
    \item Exprimer le coût moyen $C_m(x)$ (coût par tonne). Préciser $D_{C_m}$.
    \item Étudier les variations de $C_m$ sur $D_{C_m}$.
    \item Quelle production $x$ minimise le coût moyen ? Calculer ce coût minimal.
    \item Tracer la courbe de $C_m$ et interpréter graphiquement le minimum.
\end{enumerate}
\tcblower
\textbf{Solution détaillée.}

\textbf{1. Fonction coût moyen.}
$C_m(x) = \frac{C(x)}{x} = \frac{x^2 + 4x + 100}{x} = x + 4 + \frac{100}{x}$, pour $x > 0$.
$D_{C_m} = ]0\ ; +\infty[$.

\textbf{2. Variations de $C_m$.}
$C_m'(x) = 1 - \frac{100}{x^2} = \frac{x^2 - 100}{x^2} = \frac{(x-10)(x+10)}{x^2}$.
Sur $]0\ ; +\infty[$, $x^2 > 0$ et $x+10 > 0$. Signe de $C_m'$ = signe de $x-10$.
\begin{itemize}
    \item $]0\ ; 10]$ : $x-10 \le 0 \Rightarrow C_m' \le 0$ : $C_m$ \cle{décroît}.
    \item $[10\ ; +\infty[$ : $x-10 \ge 0 \Rightarrow C_m' \ge 0$ : $C_m$ \cle{croît}.
\end{itemize}
$C_m$ admet un \cle{minimum global} en $x = 10$.

\begin{center}
\begin{tabular}{|c|ccccc|}\hline
$x$ & $0$ & & $10$ & & $+\infty$ \\ \hline
$C_m'(x)$ & $-\infty$ & $-$ & $0$ & $+$ & $+$ \\ \hline
$C_m$ & $+\infty$ & $\searrow$ & $24$ & $\nearrow$ & $+\infty$ \\ \hline
\end{tabular}
\end{center}
Limites : $x \to 0^+$, $\frac{100}{x} \to +\infty \Rightarrow C_m \to +\infty$. $x \to +\infty$, $C_m(x) \sim x \to +\infty$. Asymptote oblique en $+\infty$ : $y = x + 4$.

\textbf{3. Production optimale.}
Le coût moyen est minimal pour une production de \textbf{10 tonnes/mois}.
Coût minimal : $C_m(10) = 10 + 4 + \frac{100}{10} = 24$ millions FCFA/tonne.

\textbf{4. Interprétation graphique.}
La courbe de $C_m$ a une branche infinie au voisinage de $0$ (asymptote verticale $x=0$), descend jusqu'au point $M(10\ ; 24)$, puis remonte en suivant l'asymptote oblique $y=x+4$. Le minimum correspond au point où la tangente est parallèle à l'axe des abscisses.

\begin{popfigure}
\begin{tikzpicture}[scale=0.7]
\begin{axis}[
    axis lines=middle, xlabel=$x$ (tonnes), ylabel=$C_m$ (M FCFA/t),
    xmin=0, xmax=25, ymin=0, ymax=60,
    xtick={0,5,10,15,20,25}, ytick={0,10,20,24,30,40,50},
    grid=major, grid style={popline}, clip=false
]
% Asymptote oblique
\addplot[popmuted, dashed, domain=0:25] {x+4} node[above right, pos=0.9, font=\tiny, popmuted] {$y=x+4$};
% Asymptote verticale x=0 (axe des ordonnées) déjà là
% Courbe
\addplot[popcurve, domain=0.5:24, samples=300] { x + 4 + 100/x };
% Minimum
\addplot[only marks, mark=*, poporange] coordinates {(10,24)};
\node[above, font=\small] at (axis cs:10,24) {$M(10\,;\,24)$};
\draw[poporange, dashed] (10,0) node[below, font=\small] {$10$} -- (10,24);
\draw[poporange, dashed] (0,24) node[left, font=\small] {$24$} -- (10,24);
\end{axis}
\end{tikzpicture}
\legende{Courbe du coût moyen $C_m(x) = x + 4 + \frac{100}{x}$. Minimum en $x=10$ tonnes.}
\end{popfigure}
\end{exoresolu}

\begin{methode}[Méthode générale d'étude complète d'une fonction (Bac)]
\emph{Appliquer systématiquement dans cet ordre :}
\begin{enumerate}
    \item \textbf{Ensemble de définition $D_f$} (zéros dénominateur, argument $\ln > 0$, etc.). Préciser symétries (pair/impair) et périodicité si évidente.
    \item \textbf{Limites aux bornes de $D_f$} (bornes finies interdites $\to$ asymptotes verticales ; $\pm\infty$ $\to$ asymptotes horizontales/obliques). Calculer $a = \lim \frac{f(x)}{x}$ et $b = \lim (f(x)-ax)$ pour l'oblique.
    \item \textbf{Dérivée $f'(x)$} sur $D_f$ (formules usuelles, règles de dérivation). Simplifier au maximum (factoriser).
    \item \textbf{Tableau de variations} : une colonne par valeur remarquable (bornes, zéros de $f'$, valeurs interdites doublées). Ligne $f'$ : signes. Ligne $f$ : valeurs aux bornes, flèches $\nearrow/\searrow$, images des extrema.
    \item \textbf{Points particuliers} : intersections avec les axes ($f(x)=0$, $f(0)$), intersection avec asymptotes, points d'inflexion si demandés (via $f''$).
    \item \textbf{Tracé de la courbe} : repère orthonormé, asymptotes (pointillés), points remarquables, courbe respectant variations et position par rapport aux asymptotes.
\end{enumerate}
\end{methode}

\begin{attention}[Pièges fréquents au Bac — Vigilance maximale !]
\begin{itemize}
    \item \textbf{Domaine de la dérivée :} $D_{f'} \subseteq D_f$. Ne jamais écrire $f'(x) = \frac{1}{x-2}$ pour $x \neq 2$ si $D_f = ]2\ ; +\infty[$ ; écrire $f'(x) = \frac{1}{x-2},\ x > 2$.
    \item \textbf{Tableau de variations :} Doubler la colonne des valeurs interdites (ex: $|| 1 ||$). Les flèches $\nearrow/\searrow$ vont \textbf{uniquement} sur la ligne de $f$. Les signes $+/-$ vont \textbf{uniquement} sur la ligne de $f'$.
    \item \textbf{Asymptote oblique :} Vérifier $a \neq 0$. Si $a=0$, c'est une asymptote horizontale. Ne pas oublier d'étudier la position relative (signe de $f(x)-(ax+b)$) pour le tracé.
    \item \textbf{Limites composées :} $\ln(u(x))$ : il faut $u(x) \to 0^+$ pour avoir $-\infty$, $u(x) \to +\infty$ pour $+\infty$. $\mathrm{e}^{u(x)}$ : $u(x) \to -\infty \Rightarrow 0$, $u(x) \to +\infty \Rightarrow +\infty$.
    \item \textbf{Résolution $f(x)=0$ :} Ne pas diviser par une expression qui peut s'annuler (ex: $\mathrm{e}^x$ ne s'annule jamais, on peut diviser ; $x$ peut s'annuler, ne pas diviser par $x$ sans discuter le cas $x=0$).
    \item \textbf{Unités :} Dans les problèmes, toujours conclure avec l'unité (tonnes, FCFA, mètres, etc.).
\end{itemize}
\end{attention}

\begin{savaistu}[Le saviez-vous ? — Modélisation et économie camerounaise]
La fonction coût moyen $C_m(x) = \frac{C(x)}{x}$ est un cas classique de \textbf{fonction rationnelle de type $\frac{ax^2+bx+c}{dx+e}$} (ici $d=1, e=0$). Son minimum correspond au \textbf{seuil de rentabilité optimal} (ou optimum technique). Au Cameroun, les entreprises comme \textsc{Cameroon Cocoa Corporation} ou les usines de transformation de coton (\textsc{Sodecoton}) utilisent ce type de modélisation pour dimensionner leurs chaînes de production. La forme $x + \frac{k}{x}$ (somme d'un terme croissant et d'un terme décroissant) apparaît aussi en \textbf{électrification rurale} : optimisation de la section des câbles (coût matière $\sim x$, coût pertes Joule $\sim 1/x$) pour minimiser le coût total sur la durée de vie. Les mathématiques servent concrètement à l'industrialisation du pays !
\end{savaistu}
...
\textbf{Solution :}
\begin{enumerate}
  \item $C(t) = 2e^{-0,2t} - 2e^{-0,5t}$. $D_f = [0, +\infty[$.
  Limites : $C(0)=0$. $\lim_{t\to+\infty} C(t) = 0$ (A.H. $y=0$).
  $C'(t) = 2(-0,2)e^{-0,2t} - 2(-0,5)e^{-0,5t} = -0,4e^{-0,2t} + e^{-0,5t} = e^{-0,5t}(e^{0,3t} - 0,4)$.
  $C'(t)=0 \iff e^{0,3t} = 0,4 \iff 0,3t = \ln 0,4 \iff t = \frac{\ln 0,4}{0,3} \approx \frac{-0,916}{0,3} \approx -3,05$.
  Cette valeur est négative, hors domaine $[0, +\infty[$.
  Sur $[0, +\infty[$, $e^{0,3t} \geq 1 > 0,4$, donc $C'(t) > 0$ ? Attendez.
  $e^{0,3t} - 0,4 > 0$ pour $t \geq 0$. Donc $C'(t) > 0$ sur $[0, +\infty[$.
  $C$ est strictement croissante sur $[0, +\infty[$. Pas de maximum, la concentration tend vers $0$ ? Non, limite $0$ à l'infini et $C(0)=0$, croissante $\Rightarrow$ elle augmente puis... Attendez.
  $\lim_{t\to+\infty} C(t) = 0$. $C(0)=0$. $C$ positive ? $e^{-0,2t} > e^{-0,5t}$ pour $t>0$, donc $C(t)>0$.
  Si $C$ croissante et tend vers $0$ en $+\infty$, alors $C \leq 0$. Contradiction avec $C(t)>0$.
  \textbf{Erreur de signe dans la limite} : $e^{-0,2t} \to 0$, $e^{-0,5t} \to 0$. Différence $\to 0$.
  Si $C$ croissante, $C(t) \geq C(0)=0$. Si limite $0$, alors $C(t)=0$ constant. Impossible.
  \textbf{Re-calcul dérivée} : $C'(t) = -0,4e^{-0,2t} + 1,0e^{-0,5t}$.
  $C'(t) = e^{-0,5t}(e^{0,3t} - 0,4)$.
  Pour $t=0$, $e^0 - 0,4 = 0,6 > 0$.
  Pour $t \to +\infty$, $e^{0,3t} \to +\infty$, donc $C'(t) > 0$ toujours.
  Alors $C$ croissante de $0$ vers... $0$ ? Impossible.
  \textbf{Correction limite} : $\lim_{t\to+\infty} (e^{-0,2t} - e^{-0,5t}) = 0 - 0 = 0$.
  Si $C$ croissante sur $[0,+\infty[$ et $C(0)=0$, alors $C(t) \geq 0$. Si limite $0$, alors $C(t) \to 0$ par valeurs supérieures ? Non, si croissante, $C(t) \geq C(1) > 0$ pour $t>1$, donc limite $\geq C(1) > 0$.
  \textbf{Erreur} : $e^{-0,2t}$ décroît moins vite que $e^{-0,5t}$. Pour $t$ grand, $e^{-0,2t} \gg e^{-0,5t}$. Donc $C(t) \sim 2e^{-0,2t} \to 0$.
  Mais $C$ croissante et tend vers $0$ ? Non, $C$ ne peut pas être croissante et tendre vers $0$ si $C(0)=0$ sauf si $C=0$.
  Vérifions $C(1) = 2(e^{-0,2} - e^{-0,5}) \approx 2(0,819 - 0,607) = 0,424 > 0$.
  $C(10) = 2(e^{-2} - e^{-5}) \approx 2(0,135 - 0,0067) = 0,256$.
  $C(1) > C(10)$. Donc $C$ n'est pas croissante partout.
  \textbf{Erreur sur le signe de $C'$} : $e^{0,3t} - 0,4$. Pour $t$ grand, $e^{0,3t}$ grand, positif. Pour $t=0$, $1-0,4>0$.
  Mais $C(10) < C(1)$ implique $C'$ négatif quelque part.
  $C'(t) = -0,4e^{-0,2t} + e^{-0,5t}$.
  $C'(t) > 0 \iff e^{-0,5t} > 0,4 e^{-0,2t} \iff e^{0,3t} > 0,4 \iff 0,3t > \ln 0,4 \approx -0,916$.
  Cette inéquation est vraie pour \textbf{tout} $t \geq 0$.
  Donc $C'(t) > 0$ pour tout $t \geq 0$. $C$ est strictement croissante.
  Mais $C(1) \approx 0,424$ et $C(10) \approx 0,256$. $C(10) < C(1)$.
  Calcul $C(10)$ : $e^{-2}=0,1353$, $e^{-5}=0,0067$. Diff $=0,1286$. $\times 2 = 0,257$.
  Calcul $C(1)$ : $e^{-0,2}=0,8187$, $e^{-0,5}=0,6065$. Diff $=0,2122$. $\times 2 = 0,424$.
  $C(1) > C(10)$. Donc $C$ décroît après un moment.
  Où est l'erreur ? $C'(t) = -0,4 e^{-0,2t} + 1,0 e^{-0,5t}$.
  Factorisation par $e^{-0,5t}$ : $e^{-0,5t}( -0,4 e^{0,3t} + 1 )$.
  Ah ! $e^{-0,2t} = e^{-0,5t} \cdot e^{0,3t}$.
  Donc $C'(t) = e^{-0,5t}( 1 - 0,4 e^{0,3t} )$.
  Zéro : $1 - 0,4 e^{0,3t} = 0 \iff e^{0,3t} = 2,5 \iff 0,3t = \ln 2,5 \approx 0,916 \iff t \approx 3,05$ h.
  \textbf{Voilà l'erreur classique : signe lors de la factorisation.}
  $C'(t) = e^{-0,5t}(1 - 0,4 e^{0,3t})$.
  Signe : $+$ pour $t < 3,05$, $-$ pour $t > 3,05$.
  Maximum en $t_0 \approx 3,05$ h. $C_{max} = C(3,05) \approx 2(e^{-0,61} - e^{-1,525}) \approx 2(0,543 - 0,218) = 0,65$ mg/L.
  \item Seuil $0,1$ mg/L. Résoudre $C(t) = 0,1$ sur la branche décroissante ($t > 3,05$).
  $2(e^{-0,2t} - e^{-0,5t}) = 0,1$. Posons $X = e^{-0,1t} \in ]0,1]$.
  $2(X^2 - X^5) = 0,1 \iff 20X^2 - 20X^5 = 1$. Équation en $X^3$... Résolution numérique (calculatrice) : $t \approx 14,2$ h.
  Concentration efficace pendant environ $14$ heures.
\end{enumerate}
\end{exoresolu}
