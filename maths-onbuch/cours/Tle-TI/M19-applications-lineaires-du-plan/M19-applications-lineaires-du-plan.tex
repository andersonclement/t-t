...
Ces deux droites sont \textbf{distinctes} (leurs directrices ne sont pas colinéaires) mais \textbf{pas perpendiculaires}, car $(2,1) \cdot (-2,1) = -4+1=-3 \neq 0$ : ce sont deux droites distinctes passant par l'origine.
Géométriquement, $f$ « écrase » le plan sur la droite $\operatorname{Im}(f)$ dans la direction du noyau $\operatorname{Ker}(f)$. On vérifie que $f\circ f = 4f$ : l'application $\frac{1}{4}f$ est la projection (oblique) sur $\operatorname{Im}(f)$ parallèlement à $\operatorname{Ker}(f)$.
