% Nombres complexes : forme algébrique — Mathématiques Tle TI — Cours signé OnBuch+
...
\node[fill=popblueL, align=center] {$P(z) = 0$\\ (degré 3)}
  child { node[fill=poporangeL] {Racine évidente $\alpha$\\ vérifier $P(\alpha)=0$}
    child { node[fill=popgreenL] {$P(z)=(z-\alpha)Q(z)$\\ coefficients indéterminés}
