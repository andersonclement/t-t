% Primitives d'une fonction continue — Mathématiques Tle TI — Cours signé OnBuch+
...
\pagegarde{Primitives d'une fonction continue}{Mathématiques}{Tle TI}{Relations et opérations fondamentales dans l'ensemble des nombres réels}{05}{5 h}{Cette leçon introduit la notion de primitive d'une fonction continue sur un intervalle, opération inverse de la dérivation. Elle fournit les tableaux de primitives usuelles et les techniques de reconnaissance des formes composées (u′·uʳ, u′/u, u′eᵘ) indispensables pour le calcul intégral et les équations différentielles du programme de Terminale.
\vspace{4pt}
...
Déterminer la primitive $F$ de $f$ sur $I$ qui prend la valeur $1$ en $0$... Attention, piège de lecture : $0 \notin I$ ! On travaille en réalité sur $J = \left]\dfrac{-3+\sqrt{5}}{2}\,;\,+\infty\right[$, intervalle contenant $0$ sur lequel le dénominateur ne s'annule pas. Déterminer la primitive $F$ de $f$ sur $J$ telle que $F(0) = 1$.
\tcblower
\textbf{Solution détaillée.}
\begin{enumerate}
...
\begin{enumerate}
  ...
  Primitive : $3 \cdot \frac{u^{1/2}}{1/2} = 6\sqrt{x^3+1}$.
  ...
  \item \textbf{Forme générale :} $F(x) = 6\sqrt{x^3+1} + \sin(2x) - 4\ln x + k$.
  \item \textbf{Condition initiale :} $F(1) = 6\sqrt{2} + \sin 2 - 4\ln 1 + k = 6\sqrt{2} + \sin 2 + k = 0$.
  $\Rightarrow k = -6\sqrt{2} - \sin 2$.
  \item \textbf{Primitive cherchée :}
  \[
  F(x) = 6\sqrt{x^3+1} + \sin(2x) - 4\ln x - 6\sqrt{2} - \sin 2 \quad \text{sur } ]0,+\infty[.
  \]
  \item \textbf{Vérification :} $F'(x) = 6\cdot\frac{3x^2}{2\sqrt{x^3+1}} + 2\cos(2x) - \frac{4}{x} = \frac{9x^2}{\sqrt{x^3+1}} + \dots$ \textbf{ERREUR !}
  Recalcul terme 1 : $\int 3x^2 (x^3+1)^{-1/2} dx$. $u=x^3+1$, $du=3x^2 dx$. $\int u^{-1/2} du = 2 u^{1/2} = 2\sqrt{x^3+1}$.
  Donc primitive terme 1 = $2\sqrt{x^3+1}$ (pas 6). Le facteur 3 est déjà dans $u'$.
  \textbf{Correction :} $F(x) = 2\sqrt{x^3+1} + \sin(2x) - 4\ln x + k$.
  $F(1) = 2\sqrt{2} + \sin 2 + k = 0 \Rightarrow k = -2\sqrt{2} - \sin 2$.
  $F(x) = 2\sqrt{x^3+1} + \sin(2x) - 4\ln x - 2\sqrt{2} - \sin 2$.
...
\begin{bilan}[L'essentiel de la leçon M05 — Primitives]
