#!/usr/bin/env python3
"""Pipeline de génération des cours signés OnBuch+ (Informatique, Terminale TI).
...
- Boîtes pédagogiques (titre optionnel entre crochets) :
  ...
  \begin{methode}[Titre]...\end{methode}  (démarche en étapes numérotées : ex. « comment écrire une requête SQL de jointure », « comment déployer un environnement WampServer »)
  \begin{attention}[Titre]...\end{attention}  (erreurs fréquentes, bugs classiques, failles de sécurité, pièges de syntaxe)
  \begin{experience}[Titre]...\end{experience}  (TP guidé pas à pas : objectif, matériel/logiciels, manipulation, observations, interprétation — installation d'un serveur, configuration réseau, création d'une base de données, etc.)
...
- CODE SOURCE (PHP, C, HTML, JavaScript, SQL, algorithmes/pseudo-code) : place TOUT extrait de code dans \begin{lstlisting}[language=Php]...\end{lstlisting} (adapte language à Php, C, HTML, JavaScript, SQL, ou omets l'option pour du pseudo-code générique). N'utilise JAMAIS \verb ni un simple \texttt pour un bloc de plusieurs lignes. Le code doit être complet, correct, indenté avec des espaces (pas de tabulations), et peut être commenté (// ou /* */ en PHP/C/JS, -- en SQL, <!-- --> en HTML). Un bloc de code peut être placé seul ou à l'intérieur d'une boîte pédagogique (exemplebox, methode, exoresolu, experience) : dans ce cas le \begin{lstlisting}...\end{lstlisting} figure directement dans le corps de la boîte. Pour un extrait très court en ligne (un mot-clé, un nom de fonction, une commande), utilise \texttt{motcle} plutôt qu'un bloc.
...
- Tableaux : \begin{center}\begin{tabularx}{\linewidth}{|l|X|X|}\hline ... \end{tabularx}\end{center} ou tabular + booktabs. En-têtes colorés possibles : \rowcolor{popblueL}. Utile pour : tables de vérité, plans d'adressage IP, dictionnaires de données, comparatifs de protocoles ou de SGBD.
- Illustrations : place chaque figure dans \begin{popfigure} ... \legende{Légende}\end{popfigure}. Utilise TikZ pour les SCHÉMAS D'ARCHITECTURE ET DE RÉSEAU (topologies étoile/bus/anneau, client-serveur, schémas d'équipements — routeur, switch, pare-feu —, diagrammes UML de classes/cas d'utilisation/séquence, modèles entité-association ou schémas relationnels de bases de données, organigrammes/algorigrammes d'algorithmes avec des rectangles, losanges de décision et flèches). Couleurs autorisées : popink, poporange, popdark, poppurple, popgreen, popblue, poppink, popgold, popmuted, popline, popcream, et leurs teintes popblueL, poporangeL, popgreenL, poppurpleL, poppinkL, popgoldL (ou mélanges comme popblue!30). Largeur max des figures : \linewidth. Garde le code TikZ simple, correct et compilable (bibliothèques disponibles : arrows.meta, positioning, calc, shapes.geometric, decorations.pathmorphing, patterns, mindmap, trees, fit, backgrounds, matrix, intersections). Une figure TikZ n'est pas obligatoire partout : pour montrer un extrait de code, PRÉFÈRE un bloc lstlisting bien formaté (éventuellement dans un popfigure avec \legende{} s'il s'agit d'un exemple de code à commenter comme une figure).
...
1. Exactitude technique : vérifie chaque définition, chaque syntaxe de langage (PHP, C, HTML, JavaScript, SQL), chaque extrait de code (il doit compiler/s'exécuter mentalement sans erreur, variables cohérentes, balises fermées, points-virgules présents), chaque commande ou requête, chaque protocole ou norme cité (numéros de port, en-têtes HTTP, classes d'adresses IP, mots-clés SQL). Corrige toute erreur, y compris une faute de syntaxe, un nom de fonction incorrect ou une confusion entre deux notions proches.
