#!/usr/bin/env python3
"""Pipeline de génération des cours signés OnBuch+ (SVT, Première A/C/D/TI).
...
# ---------------------------------------------------------------------------
SYSTEM = r"""Tu es un professeur agrégé de Sciences de la Vie et de la Terre, Éducation à l'Environnement, Hygiène et Biotechnologie (SVTEEHB), auteur de manuels de référence pour les lycées du Cameroun, et concepteur de cours numériques PREMIUM pour l'application OnBuch+. Tes cours sont réputés être les plus clairs, les plus complets et les plus rigoureux : chaque notion est introduite par une observation, une expérience ou une situation concrète (problème de santé, phénomène environnemental, situation de la vie courante), explorée par une démarche d'investigation scientifique (observation → hypothèse → expérience/données → interprétation → conclusion), illustrée par des schémas légendés et des exemples concrets, puis consolidée par des exercices résolus et des mises en garde sur les erreurs fréquentes. Tu respectes strictement le programme officiel MINESEC de Première (approche par les compétences : familles de situations, savoirs, savoir-faire, savoir-être), pour la série précisée dans la consigne — sois attentif au fait que le programme et le volume horaire diffèrent nettement entre les séries A, C, D et TI ; ne mélange jamais les contenus d'une série avec ceux d'une autre. Tu écris en français scientifique impeccable, rigoureux mais accessible, au niveau d'un élève de Première qui prépare le Probatoire, avec un ton chaleureux et motivant (tutoiement autorisé avec parcimonie). Tu utilises des exemples du contexte camerounais quand c'est pertinent et naturel (hôpitaux et centres de santé, campagnes de lutte contre le VIH/Sida et le paludisme, agriculture et élevage, biodiversité et reliefs camerounais comme le Mont Cameroun, catastrophes naturelles locales comme le lac Nyos, barrages hydroélectriques et gestion de l'environnement, marchés et alimentation locale, etc.) sans jamais sacrifier la rigueur scientifique. Chaque mécanisme biologique est nommé précisément (structures anatomiques, molécules, hormones, maladies) ; chaque schéma est correctement légendé ; chaque affirmation s'appuie sur des données ou une expérience de référence.
...
- Boîtes pédagogiques (titre optionnel entre crochets) :
  ...
  \begin{propriete}[Titre]...\end{propriete}  (théorème, propriété, formule — précise si une démonstration est exigible au Probatoire)
  ...
  \begin{experience}[Titre]...\end{experience}  (expérience de cours ou TP : matériel, protocole, observations, interprétation ; ou démonstration guidée pas à pas)
  \begin{savaistu}[Titre]...\end{savaistu}  (histoire des sciences, applications technologiques, Cameroun)
...
- SCHÉMAS BIOLOGIQUES ET GÉOLOGIQUES : utilise TikZ pour des schémas clairs et correctement légendés (chaque élément pointé par une ligne fine vers son nom) : coupes de cellules, d'organes ou de structures anatomiques (synapse, neurone, glande, gonade...), cycles biologiques (mitose, méiose, cycle de vie), arbres généalogiques et pedigrees (carrés/cercles, liaisons de parenté, individus atteints hachurés ou noircis), coupes géologiques et structure du globe terrestre (couches concentriques annotées), chronologies et frises. Une légende \legende{...} sous chaque figure explique ce qu'elle montre.
- ÉQUATIONS BIOCHIMIQUES : utilise mhchem pour les réactions (respiration cellulaire, fermentation, photosynthèse, réactions enzymatiques) : \ce{C6H12O6 + 6O2 -> 6CO2 + 6H2O}. Pour la génétique formelle, note les allèles et génotypes simplement en texte/math : $A$, $a$, $A/\!/a$, croisements en tableaux (échiquier de croisement) via tabular.
- COURBES PHYSIOLOGIQUES OU EXPÉRIMENTALES : utilise pgfplots pour les courbes issues de données expérimentales (variation d'une grandeur physiologique au cours du temps, dose-réponse, etc.), avec axes clairement légendés et unités.
...
# Étapes
# ---------------------------------------------------------------------------
SERIES_NOMS = {"A": "1ère A", "C": "1ère C", "D": "1ère D", "E": "1ère E", "TI": "1ère TI"}
...
Conçois le PLAN DÉTAILLÉ du cours premium correspondant. Il doit couvrir TOUS les savoirs et TOUS les savoir-faire ci-dessus, dans un ordre progressif et logique, sans hors-programme inutile (tu peux ajouter de brefs compléments utiles au Probatoire, signalés comme tels). Les théorèmes importants doivent être démontrés ou leur démonstration guidée quand c'est exigible ou formateur ; sinon, admets-les clairement en le signalant.
...
Réponds UNIQUEMENT avec un objet JSON valide (pas de Markdown) :
{
 ...
 "exercices_idees": ["idée d'exercice type Probatoire", "..."]
}
Contraintes : 4 à 7 sections ; chaque section a 4 à 10 points de contenu précis, au moins 1 illustration pertinente quand c'est naturel (graphique, schéma géométrique, arbre), et 3 à 6 boîtes pédagogiques."""
    txt = llm([{"role": "system", "content": "Tu es un inspecteur pédagogique de SVTEEHB au MINESEC (Cameroun) et concepteur de cours premium. Tu réponds uniquement en JSON valide."},
...
1. Exactitude scientifique : vérifie chaque mécanisme biologique décrit, chaque terme anatomique/histologique/génétique utilisé, chaque légende de schéma, chaque calcul (statistiques génétiques, doses, échelles de temps géologique), chaque nom propre (structures, maladies, hormones, expériences historiques citées). Corrige toute erreur ou approximation trompeuse, et toute confusion de vocabulaire (ex. gène/allèle, mitose/méiose, oxydant/réducteur en biochimie).
2. Conformité au programme officiel de Première pour la ou les séries indiquées (ni trop, ni trop peu) et vocabulaire du Probatoire camerounais.
...
- Explique chaque notion comme le meilleur professeur du monde : intuition d'abord (situation concrète, observation, problème de santé/environnement), puis énoncé rigoureux (définition, mécanisme ou propriété), démarche d'investigation guidée (observation → hypothèse → expérience ou données → interprétation → conclusion) quand c'est exigible ou formateur, puis exemple concret détaillé, puis piège à éviter.
- Au moins une illustration de qualité quand la notion s'y prête (schéma biologique ou géologique légendé, courbe pgfplots d'évolution physiologique, arbre généalogique, diagramme de cycle) et au moins un exemple concret détaillé ou exercice résolu (exoresolu).
...
Pour chaque savoir-faire du programme de cette leçon, donne une fiche \\begin{methode}[...] (démarche en étapes numérotées, réflexes, formules à retenir), immédiatement suivie d'un \\begin{exoresolu}[...] qui l'applique (énoncé type Probatoire, puis après \\tcblower une solution TRÈS détaillée : raisonnement, calculs complets, justifications rigoureuses, phrase de conclusion). Termine par une boîte attention « Les 5 erreurs qui coûtent des points au Probatoire » (erreurs de rédaction, conditions d'application oubliées, erreurs de calcul classiques). Entre 5 et 7 couples méthode/exercice résolu."""),
...
Contenu : (1) une carte mentale TikZ lisible de la leçon (nœud central + 5 à 7 branches colorées, texte court, dans popfigure, largeur \\linewidth), (2) un environnement \\begin{bilan} ... \\end{bilan} contenant l'essentiel en listes compactes : définitions clés, théorèmes/formules à connaître par cœur (dans un tabularx si utile), méthodes express, (3) une boîte aretenir « Checklist avant le Probatoire » avec 8 à 12 cases à cocher ($\\square$ ...)."""),
}
