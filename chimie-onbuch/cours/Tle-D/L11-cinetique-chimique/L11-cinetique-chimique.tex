...
\begin{corrige}[Exercices QCM]
\begin{enumerate}
\item \textbf{b.} La précipitation de l'argent chlorure \ce{AgCl(s)} est quasi instantanée (réaction rapide).
\item \textbf{c.} La corrosion du fer (formation de rouille) s'étale sur des mois ou des années : réaction lente.
\item \textbf{c.} Sans catalyseur (platine) ni étincelle, l'activation de la liaison \ce{O=O} et \ce{H-H} est quasi impossible à température ambiante : réaction très lente.
\item \textbf{b.} Le catalyseur abaisse l'énergie d'activation, accélère l'atteinte de l'équilibre, mais ne modifie ni la constante d'équilibre, ni les quantités à l'état final, et n'est pas consommé (régénéré).
\end{enumerate}
\end{corrige}
...
\begin{enumerate}
\item \textbf{Vrai.} Par définition, $v_{\text{inst}}(P) = \frac{d[P]}{dt}$, qui est la pente de la tangente à la courbe $[P]=f(t)$.
\item \textbf{Faux.} La \emph{vitesse de disparition} d'un réactif $R$ est définie par $v_{\text{disp}}(R) = -\frac{1}{\nu_R}\frac{d[R]}{dt} > 0$. C'est une grandeur positive. C'est la dérivée $\frac{d[R]}{dt}$ qui est négative.
\item \textbf{Vrai.} Pour le réactif limitant, $x_{\max}$ correspond à sa consommation totale. À $t_{1/2}$, la moitié du réactif limitant a réagi, donc $x = x_{\max}/2$.
\item \textbf{Faux.} D'après la loi d'Arrhenius, l'augmentation de la température augmente la constante de vitesse $k$ (pour la quasi-totalité des réactions), donc accélère la réaction.
\item \textbf{Faux.} L'acide sulfurique est un catalyseur (homogène). Il accélère l'atteinte de l'équilibre mais ne déplace pas l'équilibre : la quantité d'ester à l'équilibre (l'asymptote) reste inchangée.
\end{enumerate}
\end{corrige}
...
\begin{corrige}[Exercice Texte à trous]
...
\begin{corrige}[Exercice Définitions]
\begin{enumerate}
\item \textbf{Vitesse moyenne de formation d'un produit $P$} entre $t_1$ et $t_2$ : $v_{\text{moy}}(P) = \frac{[P]_{t_2} - [P]_{t_1}}{t_2 - t_1}$. Unité : \si{mol.L^{-1}.s^{-1}} (ou \si{mol.L^{-1}.min^{-1}}).
\item \textbf{Vitesse instantanée de disparition d'un réactif $R$} à la date $t$ : $v_{\text{inst, disp}}(R) = -\frac{1}{\nu_R}\frac{d[R]}{dt}$ (valeur positive). Elle est égale au coefficient directeur de la tangente à la courbe $[R]=f(t)$ (changé de signe et divisé par $|\nu_R|$). Unité : \si{mol.L^{-1}.s^{-1}}.
\item \textbf{Vitesse volumique de réaction} $v$ : $v = \frac{1}{V}\frac{dx}{dt} = \frac{1}{\nu_i}\frac{d[C_i]}{dt}$. Elle lie la variation de l'avancement $x$ (en mol) par unité de temps et par unité de volume. Unité : \si{mol.L^{-1}.s^{-1}}.
\item \textbf{Temps de demi-réaction $t_{1/2}$} : Durée nécessaire pour que l'avancement atteigne la moitié de sa valeur finale $x_{\max}$ (ou pour que la concentration du réactif limitant soit divisée par deux). Unité : \si{s} (ou \si{min}, \si{h}).
\item \textbf{Catalyseur} : Espèce chimique qui augmente la vitesse d'une réaction sans être consommée globalement (régénérée au cours du mécanisme) et sans modifier l'état final (constante d'équilibre inchangée). Il abaisse l'énergie d'activation.
\end{enumerate}
\end{corrige}
...
\begin{corrige}[Exercice Vitesse moyenne diiode]
\begin{enumerate}
\item $v_{\text{moy}}(\ce{I2}) = \frac{[\ce{I2}]_{8} - [\ce{I2}]_{2}}{8 - 2} = \frac{(4,6 - 1,8) \times 10^{-3}}{6} = \frac{2,8 \times 10^{-3}}{6} = \SI{4,67e-4}{mol.L^{-1}.min^{-1}}$.
\item $v_{\text{moy}}(\ce{I2}) = \frac{[\ce{I2}]_{10} - [\ce{I2}]_{0}}{10 - 0} = \frac{5,0 \times 10^{-3}}{10} = \SI{5,0e-4}{mol.L^{-1}.min^{-1}}$.
\item La vitesse moyenne sur les premières minutes (\SI{5,0e-4}) est supérieure à celle entre la 2\textsuperscript{ème} et la 8\textsuperscript{ème} minute (\SI{4,67e-4}). La vitesse de réaction diminue au cours du temps car les concentrations des réactifs (\ce{I-} et \ce{S2O8^2-}) diminuent, ce qui réduit la fréquence des chocs efficaces.
\end{enumerate}
\end{corrige}
...
\begin{corrige}[Exercice Vitesse volumique et avancement]
\begin{enumerate}
\item $v_{\text{moy}} = \frac{x' - x}{V(t' - t)} = \frac{(8,0 - 4,0) \times 10^{-4}}{0,100 \times (15 - 5)} = \frac{4,0 \times 10^{-4}}{1,0} = \SI{4,0e-4}{mol.L^{-1}.min^{-1}}$.
\item Pour la réaction : $v = -\frac{1}{2}\frac{d[\ce{I-}]}{dt} = -\frac{d[\ce{S2O8^2-}]}{dt} = \frac{d[\ce{I2}]}{dt}$.
    \begin{itemize}
    \item Vitesse de disparition des ions \ce{I-} : $v_{\text{disp}}(\ce{I-}) = 2 \times v = \SI{8,0e-4}{mol.L^{-1}.min^{-1}}$.
    \item Vitesse de disparition des ions \ce{S2O8^2-} : $v_{\text{disp}}(\ce{S2O8^2-}) = 1 \times v = \SI{4,0e-4}{mol.L^{-1}.min^{-1}}$.
    \end{itemize}
\item $[\ce{I2}]_{t'} = \frac{x'}{V} = \frac{8,0 \times 10^{-4}}{0,100} = \SI{8,0e-3}{mol.L^{-1}}$.
\end{enumerate}
\end{corrige}
...
\begin{corrige}[Exercice Expériences témoins]
\begin{enumerate}
\item \textbf{Exp 1 :} Réaction de référence (concentration, température). \textbf{Exp 2 :} Catalyse (ions \ce{Fe^{3+}}). \textbf{Exp 3 :} Catalyse (platine). \textbf{Exp 4 :} Surface de contact (zinc poudre vs bloc).
\item \textbf{Exp 2 :} Catalyse \textbf{homogène}. Justification : Le catalyseur \ce{Fe^{3+}} (ions en solution aqueuse) et les réactifs \ce{I-}, \ce{S2O8^{2-}} (ions en solution aqueuse) sont dans la même phase (liquide).
\item \textbf{Exp 3 :} Catalyse \textbf{hétérogène}. Justification : Le catalyseur (platine, solide) et les réactifs (\ce{H2}, \ce{O2}, gaz) sont dans des phases différentes.
\item \textbf{Non.} L'ion \ce{Fe^{3+}} est un catalyseur. Il accélère l'atteinte de l'équilibre (ou la vitesse de la réaction totale ici) mais ne modifie pas la stœchiométrie ni les quantités initiales des réactifs. La quantité finale de diiode dépend uniquement du réactif limitant, inchangée.
\end{enumerate}
\end{corrige}
...
\begin{corrige}[Exercice Courbes d'estérification]
\begin{enumerate}
\item \textbf{Courbe (a) $\leftrightarrow$ Condition (1) :} \SI{20}{\celsius} sans catalyseur. Pente initiale la plus faible, temps le plus long (\SI{50}{h}).
\textbf{Courbe (b) $\leftrightarrow$ Condition (2) :} \SI{60}{\celsius}. Température plus élevée $\rightarrow$ énergie cinétique moyenne plus grande $\rightarrow$ plus de chocs efficaces $\rightarrow$ pente initiale maximale, temps le plus court (\SI{8}{h}).
\textbf{Courbe (c) $\leftrightarrow$ Condition (3) :} \SI{20}{\celsius} avec \ce{H2SO4}. Catalyseur abaisse l'énergie d'activation $\rightarrow$ pente initiale intermédiaire, temps intermédiaire (\SI{15}{h}).
\item L'asymptote correspond à l'état d'équilibre. La constante d'équilibre $K$ ne dépend que de la température. Les trois expériences sont à \SI{20}{\celsius} (pour a et c) ou \SI{60}{\celsius} (pour b). \textbf{Attention :} L'énoncé dit "la même asymptote". Cela implique que $K$ est considéré constant (ou que l'effet de T sur K est négligé au programme pour cette illustration), ou plus probablement que l'expérience (b) est aussi ramenée à \SI{20}{\celsius} pour la comparaison finale. \emph{Au programme, on admet que le catalyseur et la température (si on compare à T constante pour l'asymptote) ne changent pas l'équilibre.}
Quantité d'ester à l'équilibre : $n_{\text{eq}} = \tau \times n_0 = 0,67 \times \SI{0,10}{mol} = \SI{0,067}{mol}$.
\item Pour déplacer l'équilibre vers l'avant (règle de Le Chatelier) :
\begin{itemize}
\item Introduire un excès de l'un des réactifs (acide ou alcool).
\item Éliminer un produit au fur et à mesure (ex: distillation de l'ester ou de l'eau, utilisation d'un agent déshydratant).
\end{itemize}
\end{enumerate}
\end{corrige}
...
\begin{corrige}[Exercice Type Bac Iodure-Peroxodisulfate]
\begin{enumerate}
\item Volume total $V = V_1 + V_2 = \SI{100,0}{mL} = \SI{0,100}{L}$.
$n_1(\ce{I-}) = C_1 V_1 = 0,50 \times 0,050 = \SI{0,0250}{mol}$.
$n_2(\ce{S2O8^2-}) = C_2 V_2 = 0,10 \times 0,050 = \SI{0,0050}{mol}$.
\begin{center}
\begin{tabular}{|c|c|c|c|c|}
\hline
 & \ce{2I-} & \ce{S2O8^2-} & \ce{I2} & \ce{2SO4^2-} \\
\hline
État initial & 0,0250 & 0,0050 & 0 & 0 \\
\hline
État final & $0,0250-2x$ & $0,0050-x$ & $x$ & $2x$ \\
\hline
\end{tabular}
\end{center}
Pour que $\ce{S2O8^2-}$ soit entièrement consommé : $x_{\max} = \SI{0,0050}{mol}$.
Il resterait $0,0250 - 2\times0,0050 = \SI{0,0150}{mol}$ d'\ce{I-}. L'ion peroxodisulfate est bien le réactif limitant. $x_{\max} = \SI{5,0e-3}{mol}$.
$[\ce{I2}]_{\max} = x_{\max}/V = \SI{0,050}{mol.L^{-1}} = \SI{50e-3}{mol.L^{-1}}$.
\item \emph{(Tracé graphique attendu : courbe croissante, concave, tendant vers 50 $\times 10^{-3}$ M).}
\item $v_{\text{moy}}(\ce{I2}) = \frac{[\ce{I2}]_{30} - [\ce{I2}]_{10}}{30 - 10} = \frac{(29,6 - 15,6) \times 10^{-3}}{20} = \frac{14,0 \times 10^{-3}}{20} = \SI{7,0e-4}{mol.L^{-1}.min^{-1}}$.
\item Tangente à $t=20$ min. Lecture graphique typique : pente $\approx \SI{6,0e-4}{mol.L^{-1}.min^{-1}}$ (valeur dépendante de la précision du tracé).
Comparaison : $v_{\text{inst}}(20\text{ min}) < v_{\text{moy}}(10-30\text{ min})$. La vitesse instantanée diminue avec le temps (réactifs consommés). La vitesse moyenne sur $[10;30]$ est une moyenne sur un intervalle où la vitesse était plus grande au début (vers 10 min) et plus petite à la fin (vers 30 min). Comme la courbe est concave, la corde est au-dessus de la tangente au milieu de l'intervalle.
\item Relation stœchiométrique : $v = \frac{d[\ce{I2}]}{dt} = -\frac{1}{2}\frac{d[\ce{I-}]}{dt} = -\frac{d[\ce{S2O8^2-}]}{dt}$.
$v_{\text{inst}}(\ce{I2}) = v_{\text{inst}} \approx \SI{6,0e-4}{mol.L^{-1}.min^{-1}}$.
$v_{\text{disp, inst}}(\ce{I-}) = 2 \times v_{\text{inst}} \approx \SI{1,2e-3}{mol.L^{-1}.min^{-1}}$.
$v_{\text{disp, inst}}(\ce{S2O8^2-}) = 1 \times v_{\text{inst}} \approx \SI{6,0e-4}{mol.L^{-1}.min^{-1}}$.
\item $t_{1/2}$ : date où $x = x_{\max}/2 = \SI{2,5e-3}{mol}$, soit $[\ce{I2}] = \SI{25e-3}{mol.L^{-1}}$.
Lecture graphique : $[\ce{I2}] = 25 \times 10^{-3}$ est atteinte entre $t=\SI{20}{min}$ ($24,4\times10^{-3}$) et $t=\SI{30}{min}$ ($29,6\times10^{-3}$). Interpolation : $t_{1/2} \approx \SI{21}{min}$.
\item \textbf{Nouvelle courbe (pointillés) :} Même asymptote ($50 \times 10^{-3}$ M), mais pente initiale plus forte, courbe plus "raide", atteignant l'asymptote plus tôt.
\textbf{Justification :} \ce{Fe^3+} catalyse la réaction (catalyse homogène), augmentant la constante de vitesse $k$.
\textbf{$t_{1/2}$ :} Diminue (atteint plus vite la moitié de l'avancement).
\textbf{Concentration finale :} Non modifiée (catalyseur ne déplace pas l'équilibre / réaction totale ici, réactif limitant inchangé).
\end{enumerate}
\end{corrige}
...
\begin{corrige}[Exercice Type Bac Eau oxygénée]
\begin{enumerate}
\item $n_0(\ce{H2O2}) = C_0 \times V_0 = 0,89 \times 20,0 \times 10^{-3} = \SI{0,0178}{mol}$.
Réaction totale : $2 \ce{H2O2} \to \ce{O2}$. $n_{\max}(\ce{O2}) = n_0(\ce{H2O2}) / 2 = \SI{0,00890}{mol}$.
\item Gaz parfait : $pV = nRT \Rightarrow n = \frac{pV}{RT}$. Ici $p = \Delta p$ (surpression due à \ce{O2}).
Attention aux unités SI : $\Delta p$ en \si{Pa} ($1 \text{ hPa} = 100 \text{ Pa}$), $V_g$ en \si{m^3} ($0,98 \text{ L} = 0,98 \times 10^{-3} \text{ m}^3$), $R = \SI{8,314}{J.mol^{-1}.K^{-1}}$, $T = 298 \text{ K}$.
$t=10 \text{ min}$ : $\Delta p = 150 \text{ hPa} = 15\,000 \text{ Pa}$.
$n(\ce{O2}) = \frac{15\,000 \times 0,98 \times 10^{-3}}{8,314 \times 298} = \frac{14,7}{2477,6} = \SI{5,93e-3}{mol}$.
$t=40 \text{ min}$ : $\Delta p = 316 \text{ hPa} = 31\,600 \text{ Pa}$.
$n(\ce{O2}) = \frac{31\,600 \times 0,98 \times 10^{-3}}{2477,6} = \frac{30,97}{2477,6} = \SI{1,25e-2}{mol}$ ? \textbf{Attention :} $n_{\max} = 0,0089$ mol. La valeur 316 hPa à 40 min semble mener à $n > n_{\max}$. Vérification : $p_{\max} = n_{\max}RT/V_g = 0,0089 \times 2477,6 / 0,98e-3 \approx 22\,500 \text{ Pa} = 225 \text{ hPa}$. Le tableau donne 316 hPa à 40 min. \textbf{Incohérence dans les données du sujet source (pression finale trop élevée).} On utilisera les valeurs du tableau pour le tracé, en notant que l'asymptote théorique est 225 hPa.
\item Avancement $x = n(\ce{O2})$ (coefficient stœchiométrique 1 devant \ce{O2}).
$n(\ce{H2O2}) = n_0 - 2x = n_0 - 2n(\ce{O2})$.
$[\ce{H2O2}] = \frac{n(\ce{H2O2})}{V_0} = \frac{n_0 - 2n(\ce{O2})}{V_0}$.
$t=10 \text{ min}$ : $n(\ce{O2}) = 5,93 \times 10^{-3}$.
$n(\ce{H2O2}) = 0,0178 - 2 \times 0,00593 = 0,00594 \text{ mol}$.
$[\ce{H2O2}] = 0,00594 / 0,020 = \SI{0,297}{mol.L^{-1}}$.
\item Tableau complet $[\ce{H2O2}]$ (en \si{mol.L^{-1}}) :
$t=0$: 0,89 ; $t=5$: 0,61 ; $t=10$: 0,30 ; $t=15$: 0,11 ; $t=20$: 0,03 ; $t=30$: $\approx 0$ ; $t=40$: $\approx 0$.
Courbe décroissante, concave, tendant vers 0.
\item Tangente à $t=15$ min sur courbe $[\ce{H2O2}]=f(t)$. Pente négative. $v_{\text{disp, inst}} = -\frac{d[\ce{H2O2}]}{dt}$ (valeur positive). Lecture graphique $\approx \SI{0,05}{mol.L^{-1}.min^{-1}}$.
\item $t_{1/2}$ : $[\ce{H2O2}] = 0,89/2 = \SI{0,445}{mol.L^{-1}}$. Lecture graphique : entre 5 et 10 min, $\approx \SI{7}{min}$.
Vérification : à $t_{1/2}$, $n(\ce{H2O2}) = n_0/2 = 0,0089$ mol. $x = n_0/4 = 0,00445$ mol. $n(\ce{O2}) = x = 0,00445$ mol.
$\Delta p = nRT/V_g = 0,00445 \times 2477,6 / 0,98e-3 \approx 11\,250 \text{ Pa} = 112,5 \text{ hPa}$.
Tableau : $\Delta p(5)=86$, $\Delta p(10)=150$. 112,5 est entre les deux $\rightarrow$ cohérent avec $t_{1/2} \approx 7$ min.
\item 1. Augmenter la température. 2. Augmenter la quantité de catalyseur (ou sa surface de contact s'il est hétérogène).
\end{enumerate}
\end{corrige}
...
\begin{corrige}[Exercice Type Bac Estérification Vie pratique]
\begin{enumerate}
\item Équation-bilan : $\ce{CH3COOH + C2H5OH <=> CH3COOC2H5 + H2O}$.
Réaction inverse : \textbf{hydrolyse de l'éthanoate d'éthyle} (hydrolyse acide ou basique).
Caractères : \textbf{Lente} (cinétique), \textbf{limitée} (équilibre, rendement < 100\%), \textbf{réversible}, \textbf{catalysée par les ions \ce{H+}}.
\item \begin{center}
\begin{tabular}{|c|c|c|c|c|}
\hline
...
\hline
État final & $0,20-x$ & $0,20-x$ & $x$ & $x$ \\
\hline
\end{tabular}
\end{center}
Réactifs en quantités stœchiométriques identiques $\rightarrow$ $x_{\max} = \SI{0,20}{mol}$.
Avancement final $x_f = n_{\infty}(\text{ester}) = \SI{0,134}{mol}$.
Taux d'avancement final $\tau = x_f / x_{\max} = 0,134 / 0,20 = 0,67 = \SI{67}{\%}$.
La réaction n'est \textbf{pas totale} ($\tau < 1$), elle est limitée par l'équilibre.
\item \textbf{Ballon B vs A :} Même température (\SI{70}{\celsius}), mais ballon B contient \ce{H2SO4} concentré $\rightarrow$ \textbf{catalyse homogène} (ions \ce{H+} en solution dans le même milieu liquide). Le catalyseur abaisse l'énergie d'activation, augmente la constante de vitesse $k$, donc vitesse initiale plus grande $\rightarrow$ asymptote atteinte plus vite (\SI{3}{h} vs \SI{12}{h}).
\textbf{Ballon C vs A/B :} Température \SI{20}{\celsius} (facteur cinétique \textbf{température}). Énergie cinétique moyenne plus faible, moins de chocs efficaces par unité de temps, constante de vitesse $k$ beaucoup plus petite $\rightarrow$ cinétique très lente (pas d'asymptote à \SI{48}{h}).
\item \textbf{Affirmation fausse.} Le rendement (taux d'avancement final $\tau$) est identique dans les ballons A et B (\SI{67}{\%}, asymptote \SI{0,134}{mol}). L'acide sulfurique n'a accéléré que la \textbf{cinétique} (vitesse), sans modifier la \textbf{thermodynamique} (position de l'équilibre, constante $K$).
\item Pour augmenter l'asymptote (déplacer l'équilibre vers l'avant, augmenter $\tau$) :
\begin{enumerate}
\item \textbf{Éliminer l'eau formée} au fur et à mesure (ex: distillation azeotropique, agent déshydratant). Selon Le Chatelier, le retrait d'un produit déplace l'équilibre vers les produits.
\item \textbf{Introduire un excès de l'un des réactifs} (acide ou alcool). L'augmentation de la concentration d'un réactif déplace l'équilibre dans le sens direct.
\end{enumerate}
\item \textbf{(a) Poisson au frais :} \textbf{Température}. Le froid (\SI{\approx 0}{\celsius}) diminue l'énergie cinétique des molécules, ralentit les réactions de décomposition bactérienne/enzymatiques (cinétique chimique et biologique), augmentant le temps de conservation.
\textbf{(b) Charbon broyé :} \textbf{Surface de contact}. Le broyage augmente la surface spécifique du solide (charbon), augmentant le nombre de sites de réaction accessibles à l'oxygène de l'air, donc la vitesse de combustion (réaction hétérogène gaz-solide).
\textbf{(c) Levure dans la pâte :} \textbf{Catalyse enzymatique (biocatalyse)}. La levure contient des enzymes (zymase) qui catalysent la fermentation alcoolique (\ce{C6H12O6 -> 2C2H5OH + 2CO2}). Les enzymes abaissent l'énergie d'activation, accélérant fortement la production de \ce{CO2} qui fait lever la pâte.
\end{enumerate}
\end{corrige}
