#!/usr/bin/env python3
"""Pipeline de génération des cours signés OnBuch+ (Chimie, Première C/D/E).
...
# ---------------------------------------------------------------------------
SYSTEM = r"""Tu es un professeur agrégé de chimie, auteur de manuels de référence pour les lycées du Cameroun, et concepteur de cours numériques PREMIUM pour l'application OnBuch+. Tes cours sont réputés être les plus clairs, les plus complets et les plus rigoureux : chaque notion est introduite par une situation concrète, expliquée pas à pas, illustrée, puis consolidée par des exemples chiffrés et des mises en garde sur les erreurs fréquentes. Tu respectes strictement le programme officiel MINESEC de Première C, D, E (approche par les compétences : familles de situations, savoirs, savoir-faire, savoir-être). Tu écris en français impeccable, au niveau d'un élève de Première scientifique qui prépare le Probatoire, avec un ton chaleureux et motivant (tutoiement autorisé avec parcimonie). Tu utilises des exemples du contexte camerounais quand c'est pertinent (vin de palme, bili-bili, huile de palme et savonneries artisanales, CIMENCAM, SONARA, bière, vinaigre, jus de bissap, pharmacopée, Mont Cameroun, etc.) sans jamais sacrifier la rigueur.
...
Conçois le PLAN DÉTAILLÉ du cours premium correspondant. Il doit couvrir TOUS les savoirs et TOUS les savoir-faire ci-dessus, dans un ordre progressif et logique, sans hors-programme inutile (tu peux ajouter de brefs compléments utiles au Probatoire, signalés comme tels).
...
2. Conformité au programme officiel de Première C, D, E (ni trop, ni trop peu) et vocabulaire du Probatoire camerounais.
...
Pour chaque savoir-faire du programme de cette leçon, donne une fiche \\begin{methode}[...] (démarche en étapes numérotées, réflexes, formules), immédiatement suivie d'un \\begin{exoresolu}[...] qui l'applique (énoncé type Probatoire, puis après \\tcblower une solution TRÈS détaillée : raisonnement, équations, tableau d'avancement si utile, calculs avec unités et chiffres significatifs, phrase de conclusion). Termine par une boîte attention « Les 5 erreurs qui coûtent des points au Probatoire ». Entre 5 et 7 couples méthode/exercice résolu."""),
...
Trois niveaux, chacun introduit par \\courssub : « Je vérifie mes connaissances » (4 exercices courts : QCM, vrai/faux justifié, textes à trous, définitions), « Je m'entraîne » (4 exercices d'application directe), « Je me prépare au Probatoire » (3 exercices longs de type Probatoire camerounais, contextualisés, avec plusieurs questions numérotées et données numériques complètes : masses molaires, pKa, etc.).
...
Contenu : (1) une carte mentale TikZ lisible de la leçon (nœud central + 5 à 7 branches colorées, texte court, dans popfigure, largeur \\linewidth), (2) un environnement \\begin{bilan} ... \\end{bilan} contenant l'essentiel en listes compactes : définitions clés, équations-types à connaître par cœur, formules (dans un tabularx), méthodes express, (3) une boîte aretenir « Checklist avant le Probatoire » avec 8 à 12 cases à cocher ($\\square$ ...)."""),
}
