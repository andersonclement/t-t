...
Les programmes de Première C, D, E, TI sont identiques : le contenu est le
même, seul le niveau affiché (page de garde, en-tête) change.

Sortie : cours/1ere-<S>/<id>-<slug>/<id>-<slug>.tex et .pdf
Chaque .tex est autonome (préambule inclus) ; il se compile depuis son
dossier avec `tectonic <fichier>.tex` (polices dans chimie-onbuch/fonts/).
"""
...
def assemble(L, serie):
    ...
    extras = [(t, re.sub(r"\\begin\{corrige\}.*?\\end\{corrige\}\s*", "", c, flags=re.S) if t == "Exercices" else c)
              for t, c in extras]
    rel_fonts = os.path.relpath(ROOT / "fonts", OUT / f"1ere-{serie}" / f"{L['id']}-{L['slug']}") + "/"
    ...
    tex = (
        ...
        f"% Programme officiel MINESEC, Première C, D, E, TI. Compiler avec : tectonic {L['id']}-{L['slug']}.tex\n"
        f"\\newcommand{{\\DOCMATIERE}}{{Chimie}}\n\\newcommand{{\\DOCNIVEAU}}{{{niveau}}}\n"
        f"\\newcommand{{\\DOCMODULE}}{{{esc(L['module'])}}}\n\\newcommand{{\\DOCLECON}}{{{num}}}\n"
        f"\\newcommand{{\\DOCTITRE}}{{{esc(L['titre'])}}}\n"
        + preamble + "\n\\begin{document}\n\n" + "\n\n".join(body) + "\n\n\\end{document}\n")
    out = OUT / f"1ere-{serie}" / f"{L['id']}-{L['slug']}"
...
def main():
    ...
    for L in cat["lessons"]:
        ...
        if not (BUILD / L["id"] / "DONE").exists():
            ...
            continue
        for s in L.get("series", cat["series"]):
