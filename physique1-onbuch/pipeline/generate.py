#!/usr/bin/env python3
"""Pipeline de génération des cours signés OnBuch+ (Physique, Première C).
...
# ---------------------------------------------------------------------------
SYSTEM = r"""Tu es un professeur agrégé de physique, auteur de manuels de référence pour les lycées du Cameroun, et concepteur de cours numériques PREMIUM pour l'application OnBuch+. Tes cours sont réputés être les plus clairs, les plus complets et les plus rigoureux : chaque notion est introduite par une observation, une expérience ou une situation concrète, modélisée et justifiée proprement (lois, bilans de forces, équations différentielles, analyse dimensionnelle), illustrée par des exemples chiffrés et des figures, puis consolidée par des exercices résolus et des mises en garde sur les erreurs fréquentes. Tu respectes strictement le programme officiel MINESEC de Première (approche par les compétences : familles de situations, savoirs, savoir-faire, savoir-être), pour la série précisée dans la consigne. Tu écris en français scientifique impeccable, rigoureux mais accessible, au niveau d'un élève de Première qui prépare le Probatoire, avec un ton chaleureux et motivant (tutoiement autorisé avec parcimonie). Tu utilises des exemples du contexte camerounais quand c'est pertinent et naturel (barrages de Lom Pangar, Nachtigal et Edéa, électrification rurale, SONATREL/ENEO, télécommunications MTN/Orange/CAMTEL, satellites, mototaxis, football, agriculture, hôpitaux et radiothérapie, etc.) sans jamais sacrifier la rigueur physique. Chaque résultat numérique est accompagné de son unité SI et d'un nombre raisonnable de chiffres significatifs ; chaque loi est vérifiée par analyse dimensionnelle quand c'est utile ; les vecteurs forces sont représentés sur des schémas soignés.
...
Contraintes : 4 à 7 sections ; chaque section a 4 à 10 points de contenu précis, au moins 1 illustration pertinente quand c'est naturel (graphique, schéma géométrique, arbre), et 3 à 6 boîtes pédagogiques."""
    txt = llm([{"role": "system", "content": "Tu es un inspecteur pédagogique de physique au MINESEC (Cameroun) et concepteur de cours premium. Tu réponds uniquement en JSON valide."},
...
1. Exactitude scientifique : vérifie chaque loi, chaque bilan de forces, chaque équation différentielle et sa solution, chaque calcul, chaque valeur numérique, chaque unité et chaque ordre de grandeur (constantes : G = 6,67e-11, e = 1,6e-19 C, h = 6,63e-34 J.s, c = 3,00e8 m/s, me = 9,11e-31 kg). Corrige toute erreur, y compris de signe, d'indice ou de condition d'application oubliée.
...
- Explique chaque notion comme le meilleur professeur du monde : intuition d'abord, puis énoncé rigoureux (définition, loi ou propriété), établissement des relations (bilan des forces, lois de Newton, équation différentielle, bilan énergétique) quand c'est exigible ou formateur, puis exemple chiffré détaillé, puis piège à éviter.
- Au moins une illustration de qualité quand la notion s'y prête (schéma de l'expérience avec vecteurs forces/champs, schéma électrique circuitikz, courbe pgfplots d'évolution temporelle, diagramme énergétique, trajectoire) et au moins un exemple chiffré ou exercice résolu (exoresolu).
