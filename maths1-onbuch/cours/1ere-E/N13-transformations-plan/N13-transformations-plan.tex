% Transformations du plan : définitions analytiques et composées — Mathématiques 1ère E — Cours signé OnBuch+
...
\pagegarde{Transformations du plan : définitions analytiques et composées}{Mathématiques}{1ère E}{Configurations et transformations élémentaires du plan}{N13}{14 h}{Cette leçon établit les définitions analytiques rigoureuses des translations et homothéties dans un repère, puis explore la structure algébrique des composées de transformations élémentaires (rotations, homothéties, translations) pour résoudre des problèmes géométriques complexes.
\vspace{4pt}
...
\begin{definition}[Transformation du plan]
Une \cle{transformation du plan} est une application $f$ du plan $\mathcal{P}$ dans lui-même qui est \textbf{bijective} (à la fois injective et surjective).
\begin{itemize}
  \item \textbf{Injective} : deux points distincts ont des images distinctes ($M \neq N \implies f(M) \neq f(N)$).
  \item \textbf{Surjective} : tout point du plan a au moins un antécédent ($\forall M' \in \mathcal{P},\ \exists M \in \mathcal{P},\ f(M)=M'$).
\end{itemize}
La bijection garantit l'existence d'une \cle{réciproque} $f^{-1}$, qui est aussi une transformation du plan, telle que $f \circ f^{-1} = f^{-1} \circ f = \mathrm{Id}_{\mathcal{P}}$.
\end{definition}

\begin{propriete}[Définition analytique dans un repère]
Dans un repère orthonormé $(O;\vec{\imath},\vec{\jmath})$, une transformation $f$ se définit par deux fonctions $X(x,y)$ et $Y(x,y)$ donnant les coordonnées de l'image $M'(X,Y)$ du point $M(x,y)$ :
\[ f : \begin{cases} X = \varphi(x,y) \\ Y = \psi(x,y) \end{cases} \]
$f$ est une transformation \ssi le système admet une unique solution $(x,y)$ pour tout couple $(X,Y)$ (la matrice jacobienne est inversible partout, ou on résout explicitement).
La réciproque s'obtient en exprimant $x,y$ en fonction de $X,Y$.
\end{propriete}

\begin{aretenir}[Vocabulaire du Probatoire]
\begin{itemize}
  \item \textbf{Image} : $M'$ image de $M$ par $f$ ; on note $f(M)=M'$.
  \item \textbf{Antécédent} : $M$ antécédent de $M'$ par $f$ ; on note $f^{-1}(M')=M$.
  \item \textbf{Point fixe} : $M$ tel que $f(M)=M$.
  \item \textbf{Composée} : $g \circ f$ signifie « on applique $f$ \emph{puis} $g$ » : $(g \circ f)(M) = g(f(M))$.
  \item \textbf{Éléments caractéristiques} : centre/angle pour une rotation ; centre/rapport pour une homothétie ; vecteur pour une translation ; axe pour une symétrie.
\end{itemize}
\end{aretenir}

\begin{exemplebox}[Vérifier qu'une application est une transformation]
Soit $f$ définie par $\begin{cases} X = 2x + y \\ Y = x - y \end{cases}$.
\textbf{Résolution :} On exprime $x,y$ en fonction de $X,Y$.
$X+Y = 3x \implies x = \frac{X+Y}{3}$.
$X - 2Y = 3y \implies y = \frac{X-2Y}{3}$.
Comme on a trouvé une unique solution pour tout $(X,Y)$, $f$ est bijective. C'est une transformation.
Sa réciproque est $f^{-1} : \begin{cases} x = \frac{X+Y}{3} \\ y = \frac{X-2Y}{3} \end{cases}$.
\end{exemplebox}

\coursec{Définition analytique de la translation}

\begin{definition}[Translation de vecteur $\vec{u}$]
Soit $\vec{u}(a\,;b)$ un vecteur non nul. La \cle{translation} de vecteur $\vec{u}$, notée $t_{\vec{u}}$, est la transformation qui à tout point $M$ associe le point $M'$ tel que $\overrightarrow{MM'} = \vec{u}$.
\end{definition}

\begin{propriete}[Écriture analytique]
Dans un repère $(O;\vec{\imath},\vec{\jmath})$, si $\vec{u}(a\,;b)$, la translation $t_{\vec{u}}$ s'écrit :
\[ t_{\vec{u}} : \begin{cases} X = x + a \\ Y = y + b \end{cases} \quad \text{ou encore} \quad \begin{cases} X - x = a \\ Y - y = b \end{cases} \]
\end{propriete}

\begin{propriete}[Réciproque d'une translation]
La réciproque de $t_{\vec{u}}$ est la translation de vecteur opposé : $t_{\vec{u}}^{-1} = t_{-\vec{u}}$.
Analytiquement : $\begin{cases} x = X - a \\ y = Y - b \end{cases}$.
\end{propriete}

\begin{aretenir}[Reconnaître une translation]
Une application $f : \begin{cases} X = x + a \\ Y = y + b \end{cases}$ avec $(a,b) \neq (0,0)$ est \textbf{nécessairement} la translation de vecteur $\vec{u}(a\,;b)$.
Si $a=b=0$, c'est l'identité.
\end{aretenir}

\begin{methode}[Déterminer le vecteur d'une translation donnée analytiquement]
\begin{enumerate}
  \item L'écriture sous la forme $X = x + a,\ Y = y + b$.
  \item Le vecteur est $\vec{u}(a\,;b)$.
  \item Vérifier que ce n'est pas l'identité ($a,b$ non tous nuls).
\end{enumerate}
\end{methode}

\begin{exoresolu}[Translation et réciproque]
Dans un repère, $f$ est définie par $\begin{cases} X = x - 3 \\ Y = y + 2 \end{cases}$.
\begin{enumerate}
  \item Montrer que $f$ est une translation et donner son vecteur.
  \item Définir analytiquement $f^{-1}$.
\end{enumerate}
\tcblower
\textbf{Solution.}
\begin{enumerate}
  \item On a $X - x = -3$ et $Y - y = 2$. C'est l'écriture analytique d'une translation de vecteur $\vec{u}(-3\,;2)$.
  \item $f^{-1}$ est la translation de vecteur $-\vec{u}(3\,;-2)$ : $\begin{cases} x = X + 3 \\ y = Y - 2 \end{cases}$.
\end{enumerate}
\end{exoresolu}

\coursec{Définition analytique de l'homothétie}

\begin{definition}[Homothétie de centre $\Omega$ et rapport $k$]
Soit $\Omega$ un point du plan et $k \in \mathbb{R}^*$ un nombre non nul. L'\cle{homothétie} de centre $\Omega$ et de rapport $k$, notée $h_{\Omega,k}$, est la transformation qui à tout point $M$ associe le point $M'$ tel que $\overrightarrow{\Omega M'} = k\,\overrightarrow{\Omega M}$.
\begin{itemize}
  \item Si $k > 0$, $M'$ est sur la demi-droite $[\Omega M)$.
  \item Si $k < 0$, $M'$ est sur la droite $(\Omega M)$ de l'autre côté de $\Omega$ (homothétie-symétrie centrale pour $k=-1$).
\end{itemize}
\end{definition}

\begin{propriete}[Écriture analytique]
Dans un repère $(O;\vec{\imath},\vec{\jmath})$, soit $\Omega(a\,;b)$ et $k \in \mathbb{R}^*$. L'homothétie $h_{\Omega,k}$ s'écrit :
\[ h_{\Omega,k} : \begin{cases} X - a = k(x - a) \\ Y - b = k(y - b) \end{cases} \quad \text{ou} \quad \begin{cases} X = kx + (1-k)a \\ Y = ky + (1-k)b \end{cases} \]
\end{propriete}

\begin{propriete}[Réciproque d'une homothétie]
La réciproque de $h_{\Omega,k}$ est l'homothétie de même centre et de rapport inverse : $h_{\Omega,k}^{-1} = h_{\Omega,\frac{1}{k}}$.
Analytiquement : $\begin{cases} x = \frac{1}{k}X + (1-\frac{1}{k})a \\ y = \frac{1}{k}Y + (1-\frac{1}{k})b \end{cases}$.
\end{propriete}

\begin{aretenir}[Reconnaître une homothétie]
Une application $f : \begin{cases} X = kx + c \\ Y = ky + c' \end{cases}$ avec $k \neq 0,1$ est \textbf{nécessairement} une homothétie de rapport $k$.
Son centre $\Omega(a\,;b)$ est le point fixe (solution de $X=x, Y=y$) :
\[ a = \frac{c}{1-k},\quad b = \frac{c'}{1-k}. \]
Si $k=1$ et $(c,c')\neq(0,0)$, c'est une translation. Si $k=1$ et $c=c'=0$, c'est l'identité.
\end{aretenir}

\begin{attention}[Piège : $k=1$ et $k=0$]
\begin{itemize}
  \item $k=0$ n'est \textbf{pas} une homothétie (pas bijective : tout le plan se projette sur $\Omega$).
  \item $k=1$ donne l'identité (cas particulier d'homothétie souvent exclu de la définition car centre indéterminé).
\end{itemize}
\end{attention}

\begin{exoresolu}[Homothétie : centre, rapport, réciproque]
$f : \begin{cases} X = 2x - 1 \\ Y = 2y + 3 \end{cases}$.
\begin{enumerate}
  \item Montrer que $f$ est une homothétie. Donner son rapport et son centre.
  \item Écrire $f^{-1}$.
\end{enumerate}
\tcblower
\textbf{Solution.}
\begin{enumerate}
  \item Forme $X = kx + c$, $Y = ky + c'$ avec $k=2$, $c=-1$, $c'=3$. $k \neq 0,1$ : c'est une homothétie de rapport $2$.
  Centre $\Omega$ : $a = \frac{-1}{1-2} = 1$, $b = \frac{3}{1-2} = -3$. Donc $\Omega(1\,;-3)$.
  \item $f^{-1} = h_{\Omega,\frac12}$ : $\begin{cases} x = \frac12 X + \frac12 \\ y = \frac12 Y - \frac32 \end{cases}$.
\end{enumerate}
\end{exoresolu}

\coursec{Outils : Composées de symétries orthogonales (Rappels et unification)}

\begin{savaistu}[La clé de voûte : la symétrie orthogonale]
Toute isométrie du plan (translation, rotation, symétrie orthogonale, symétrie glissée) se décompose en \textbf{produit de symétries orthogonales}. C'est le théorème de Cartan-Dieudonné (version plane). Maîtriser les composées de deux symétries orthogonales permet de tout démontrer sur les rotations et translations.
\end{savaistu}

\begin{propriete}[Composée de deux symétries orthogonales d'axes parallèles]
Soient $(\Delta_1)$ et $(\Delta_2)$ deux droites \textbf{strictement parallèles}. Soient $S_1, S_2$ les symétries orthogonales d'axes $(\Delta_1), (\Delta_2)$.
Alors $S_2 \circ S_1$ est la \textbf{translation} de vecteur $\vec{u} = 2\,\overrightarrow{H_1H_2}$, où $H_1 \in (\Delta_1)$ et $H_2$ est le projeté orthogonal de $H_1$ sur $(\Delta_2)$.
Le vecteur $\vec{u}$ est \textbf{normal} aux axes, orienté de $(\Delta_1)$ vers $(\Delta_2)$, de norme $2d((\Delta_1),(\Delta_2))$.
\end{propriete}

\begin{propriete}[Composée de deux symétries orthogonales d'axes sécants]
Soient $(\Delta_1)$ et $(\Delta_2)$ deux droites \textbf{sécantes} en $\Omega$. Soit $\alpha = \angle((\Delta_1),(\Delta_2))$ l'angle orienté.
Alors $S_2 \circ S_1$ est la \textbf{rotation} de centre $\Omega$ et d'angle $2\alpha$ : $r_{\Omega,\,2\alpha}$.
\end{propriete}

\begin{attention}[Ordre de la composition !]
$S_2 \circ S_1$ signifie « $S_1$ \emph{puis} $S_2$ ».
\begin{itemize}
  \item Axes parallèles : $S_2 \circ S_1 = t_{\vec{u}}$ avec $\vec{u}$ orienté de $(\Delta_1)$ vers $(\Delta_2)$. $S_1 \circ S_2 = t_{-\vec{u}}$.
  \item Axes sécants : $S_2 \circ S_1 = r_{\Omega,2\alpha}$ où $\alpha = \angle((\Delta_1),(\Delta_2))$. $S_1 \circ S_2 = r_{\Omega,-2\alpha}$.
\end{itemize}
\textbf{Règle mnémotechnique :} On double l'angle du \textbf{premier} axe vers le \textbf{second}.
\end{attention}

\begin{popfigure}
\begin{tikzpicture}[scale=1.2]
  % Axes parallèles
  \draw[popblue,thick] (0,-0.5) -- (0,2.5) node[above]{$(\Delta_1)$};
  \draw[poporange,thick] (1.5,-0.5) -- (1.5,2.5) node[above]{$(\Delta_2)$};
  \fill[popink] (-0.8,1) circle(1.8pt) node[left]{$M$};
  \fill[popgreen] (0.8,1) circle(1.8pt) node[above right]{$M_1=S_1(M)$};
  \fill[poppurple] (2.3,1) circle(1.8pt) node[right]{$M'=S_2(M_1)$};
  \draw[->,popdark,thick] (-0.8,0.5) -- (2.3,0.5) node[midway,below]{$\vec{u}=2\overrightarrow{H_1H_2}$};
  \draw[<->,popmuted] (0,2.1) -- (1.5,2.1) node[midway,above]{$d$};
  \draw[dashed,popmuted] (-0.8,1) -- (0,1);
  \draw[dashed,popmuted] (0.8,1) -- (1.5,1);
  \draw[dashed,popmuted] (2.3,1) -- (1.5,1);
  \node[popblue] at (0,2.7) {$S_1$};
  \node[poporange] at (1.5,2.7) {$S_2$};
\end{tikzpicture}
\legende{Composée $S_2 \circ S_1$ : translation de vecteur $\vec{u}$ normal aux axes, orienté $\Delta_1 \to \Delta_2$, norme $2d$.}
\end{popfigure}

\begin{methode}[Décomposer une translation ou une rotation en deux symétries]
\begin{enumerate}
  \item \textbf{Translation $t_{\vec{u}}$} : Choisir une droite $(\Delta_1)$ \textbf{perpendiculaire} à $\vec{u}$ (arbitraire). Soit $H_1 \in (\Delta_1)$. Construire $H_2$ tel que $\overrightarrow{H_1H_2} = \frac12\vec{u}$. $(\Delta_2)$ est la parallèle à $(\Delta_1)$ passant par $H_2$. Alors $t_{\vec{u}} = S_{\Delta_2} \circ S_{\Delta_1}$.
  \item \textbf{Rotation $r_{\Omega,\theta}$} : Choisir une droite $(\Delta_1)$ passant par $\Omega$ (arbitraire). Construire $(\Delta_2)$ passant par $\Omega$ telle que $\angle((\Delta_1),(\Delta_2)) = \frac{\theta}{2}$. Alors $r_{\Omega,\theta} = S_{\Delta_2} \circ S_{\Delta_1}$.
\end{enumerate}
\end{methode}
...
Soient $r_1 = r_{\Omega,\alpha}$ et $r_2 = r_{\Omega,\beta}$ deux rotations de \textbf{même centre} $\Omega$.
Leur composée $r_2 \circ r_1$ est la \textbf{rotation} de centre $\Omega$ et d'angle $\alpha + \beta$ (modulo $2\pi$) :
\[ r_{\Omega,\beta} \circ r_{\Omega,\alpha} = r_{\Omega,\,\alpha+\beta}. \]
En particulier, la réciproque de $r_{\Omega,\alpha}$ est $r_{\Omega,-\alpha}$.
\end{propriete}

\begin{experience}[Démonstration par décomposition en symétries]
On décompose chaque rotation en deux symétries orthogonales en choisissant \textbf{le même premier axe} $(\Delta_1)$ passant par $\Omega$.
\begin{enumerate}
  \item $r_{\Omega,\alpha} = S_{\Delta_2} \circ S_{\Delta_1}$ avec $\angle((\Delta_1),(\Delta_2)) = \frac{\alpha}{2}$.
  \item $r_{\Omega,\beta} = S_{\Delta_3} \circ S_{\Delta_2}$ avec $\angle((\Delta_2),(\Delta_3)) = \frac{\beta}{2}$ (on choisit $(\Delta_2)$ comme axe commun !).
  \item $r_{\Omega,\beta} \circ r_{\Omega,\alpha} = (S_{\Delta_3} \circ S_{\Delta_2}) \circ (S_{\Delta_2} \circ S_{\Delta_1}) = S_{\Delta_3} \circ (S_{\Delta_2} \circ S_{\Delta_2}) \circ S_{\Delta_1} = S_{\Delta_3} \circ \mathrm{Id} \circ S_{\Delta_1} = S_{\Delta_3} \circ S_{\Delta_1}$.
  \item Or $\angle((\Delta_1),(\Delta_3)) = \frac{\alpha}{2} + \frac{\beta}{2} = \frac{\alpha+\beta}{2}$. Donc $S_{\Delta_3} \circ S_{\Delta_1} = r_{\Omega,\alpha+\beta}$.
\end{enumerate}
\end{experience}

\begin{exemplebox}[Application : Polygone régulier]
Les rotations successives de centre $O$ et d'angle $\frac{2\pi}{n}$ engendrent le groupe des rotations du polygone régulier à $n$ côtés. La composée de $k$ telles rotations est la rotation d'angle $k\frac{2\pi}{n}$.
\end{exemplebox}

\coursec{Composée de deux homothéties de même centre}

\begin{propriete}[Composée de deux homothéties de même centre]
Soient $h_1 = h_{\Omega,k}$ et $h_2 = h_{\Omega,k'}$ deux homothéties de \textbf{même centre} $\Omega$, de rapports $k,k' \in \mathbb{R}^*$.
Leur composée $h_2 \circ h_1$ est l'\textbf{homothétie} de centre $\Omega$ et de rapport $kk'$ :
\[ h_{\Omega,k'} \circ h_{\Omega,k} = h_{\Omega,\,kk'}. \]
En particulier, la réciproque de $h_{\Omega,k}$ est $h_{\Omega,\frac{1}{k}}$.
\end{propriete}

\begin{experience}[Démonstration analytique]
Dans un repère d'origine $\Omega$ (donc $\Omega(0,0)$) :
$h_1 : \begin{cases} X = kx \\ Y = ky \end{cases}$ ; $h_2 : \begin{cases} X' = k'X \\ Y' = k'Y \end{cases}$.
Composée : $X' = k'(kx) = (kk')x$, $Y' = (kk')y$. C'est bien $h_{\Omega,kk'}$.
Pour un centre $\Omega(a,b)$ quelconque, le calcul sur les vecteurs $\overrightarrow{\Omega M}$ donne le même résultat car $\overrightarrow{\Omega M''} = k'\overrightarrow{\Omega M'} = k'(k\overrightarrow{\Omega M}) = (kk')\overrightarrow{\Omega M}$.
\end{experience}

\begin{aretenir}[Cas particuliers]
\begin{itemize}
  \item $k' = \frac{1}{k}$ : composée = identité.
  \item $k' = -1, k = -1$ : composée = homothétie de rapport $1$ = identité (symétrie centrale composée avec elle-même).
  \item $k' = -1, k \neq -1$ : composée = homothétie de rapport $-k$ (rotation de $\pi$ + homothétie).
\end{itemize}
\end{aretenir}

\coursec{Composée d'une homothétie et d'une translation}

\begin{propriete}[Composée $h_{\Omega,k} \circ t_{\vec{u}}$ et $t_{\vec{u}} \circ h_{\Omega,k}$]
Soit $h = h_{\Omega,k}$ ($k \neq 1$) et $t = t_{\vec{u}}$.
\begin{enumerate}
  \item $h \circ t$ est l'\textbf{homothétie} de rapport $k$ et de centre $\Omega'$, image de $\Omega$ par la translation de vecteur $\frac{1}{1-k}\vec{u}$.
  \item $t \circ h$ est l'\textbf{homothétie} de rapport $k$ et de centre $\Omega''$, image de $\Omega$ par la translation de vecteur $\frac{k}{1-k}\vec{u}$ (ou $-\frac{1}{1-k}\vec{u}$ selon conventions, vérifier par point fixe).
\end{enumerate}
Si $k=1$, $h$ est l'identité, la composée est la translation $t_{\vec{u}}$.
\end{propriete}

\begin{experience}[Démonstration analytique (méthode universelle)]
Dans un repère, $\Omega(a,b)$, $\vec{u}(u_1,u_2)$.
$t_{\vec{u}} : \begin{cases} X = x + u_1 \\ Y = y + u_2 \end{cases}$.
$h_{\Omega,k} : \begin{cases} X' = kX + (1-k)a \\ Y' = kY + (1-k)b \end{cases}$.
Composée $h \circ t$ : $X' = k(x+u_1) + (1-k)a = kx + [ku_1 + (1-k)a]$.
C'est de la forme $X' = kx + C$. Comme $k \neq 1$, c'est une homothétie de rapport $k$.
Son centre $\Omega'(a',b')$ est le point fixe :
$a' = ka' + ku_1 + (1-k)a \implies (1-k)a' = (1-k)a + ku_1 \implies a' = a + \frac{k}{1-k}u_1$.
Donc $\overrightarrow{\Omega\Omega'} = \frac{k}{1-k}\vec{u}$.
\textbf{Attention :} L'ordre change le centre ! Faire le calcul pour $t \circ h$ donne $\overrightarrow{\Omega\Omega''} = \frac{1}{1-k}\vec{u}$.
\end{experience}

\begin{methode}[Déterminer la nature et les éléments de $h \circ t$ ou $t \circ h$]
\begin{enumerate}
  \item Écrire les définitions analytiques de $h$ et $t$.
  \item Composer les formules (substituer).
  \item Identifier la forme : $X' = kx + C$, $Y' = ky + C'$.
  \item Si $k \neq 1$ : c'est une \textbf{homothétie de rapport $k$}. Centre = point fixe (résoudre $x=X', y=Y'$).
  \item Si $k = 1$ : c'est une \textbf{translation} de vecteur $(C,C')$.
\end{enumerate}
\end{methode}

\begin{exoresolu}[Homothétie et translation]
$\Omega(1,2)$, $k=3$, $\vec{u}(2,-1)$. Déterminer $f = h_{\Omega,3} \circ t_{\vec{u}}$.
\tcblower
\textbf{Solution.}
$t : \begin{cases} X = x+2 \\ Y = y-1 \end{cases}$.
$h : \begin{cases} X' = 3X - 2 \\ Y' = 3Y - 4 \end{cases}$ (car $(1-k)a = -2\cdot1=-2$, $(1-k)b = -2\cdot2=-4$).
$f = h \circ t : \begin{cases} X' = 3(x+2)-2 = 3x+4 \\ Y' = 3(y-1)-4 = 3y-7 \end{cases}$.
Rapport $k=3 \neq 1$ : homothétie de rapport $3$.
Centre $\Omega'$ : $x = 3x+4 \implies x=-2$ ; $y = 3y-7 \implies y=3,5$.
$\Omega'(-2\,;3,5)$. Vérification : $\overrightarrow{\Omega\Omega'} = (-3\,;1,5) = \frac{3}{1-3}(2,-1) = \frac{3}{-2}(2,-1) = (-3\,;1,5)$. \textbf{OK}.
\end{exoresolu}

\coursec{Composée d'une homothétie et d'une rotation de même centre (Similitude directe)}

\begin{definition}[Similitude directe]
Une \cle{similitude directe} est une transformation du plan qui conserve les angles orientés et multiplie toutes les longueurs par un même rapport $k > 0$ (le \cle{rapport de similitude}).
C'est la composée d'une homothétie et d'une rotation de \textbf{même centre} (l'ordre n'importe pas car elles commutent).
\end{definition}

\begin{propriete}[Composée $h_{\Omega,k} \circ r_{\Omega,\theta} = r_{\Omega,\theta} \circ h_{\Omega,k}$]
Soit $h = h_{\Omega,k}$ ($k>0$) et $r = r_{\Omega,\theta}$.
Leur composée (dans n'importe quel ordre) est une \textbf{similitude directe} de centre $\Omega$, de rapport $k$ et d'angle $\theta$.
On la note souvent $S_{\Omega,k,\theta}$ ou simplement $h_{\Omega,k} \circ r_{\Omega,\theta}$.
\end{propriete}

\begin{experience}[Démonstration géométrique]
Soit $M$ un point, $M_1 = r(M)$, $M' = h(M_1)$.
$\overrightarrow{\Omega M_1}$ est $\overrightarrow{\Omega M}$ tourné de $\theta$.
$\overrightarrow{\Omega M'} = k\,\overrightarrow{\Omega M_1}$ est $\overrightarrow{\Omega M}$ tourné de $\theta$ et multiplié par $k$.
Si on fait $h$ puis $r$ : $\overrightarrow{\Omega M_1} = k\,\overrightarrow{\Omega M}$, puis rotation de $\theta$ : même résultat.
Donc les deux composées coïncident. C'est une similitude directe.
\end{experience}

\begin{propriete}[Écriture analytique d'une similitude directe]
Dans un repère d'origine $\Omega$ (ou en coordonnées vectorielles par rapport à $\Omega$), une similitude directe de rapport $k>0$ et d'angle $\theta$ s'écrit :
\[ \begin{cases} X = k(x\cos\theta - y\sin\theta) \\ Y = k(x\sin\theta + y\cos\theta) \end{cases} \]
Sous forme complexe (si $z=x+iy$, $Z=X+iY$) : $Z = k e^{i\theta} z$.
Dans un repère quelconque d'origine $O$, avec $\Omega(a,b)$ :
\[ \begin{cases} X - a = k[(x-a)\cos\theta - (y-b)\sin\theta] \\ Y - b = k[(x-a)\sin\theta + (y-b)\cos\theta] \end{cases} \]
\end{propriete}

\begin{aretenir}[Réciproque d'une similitude directe]
La réciproque de $S_{\Omega,k,\theta}$ est la similitude directe $S_{\Omega,\frac{1}{k},-\theta}$.
Analytiquement : on divise par $k$ et on tourne de $-\theta$.
\end{aretenir}

\begin{exemplebox}[Similitude directe : construction du centre]
Soit $f = h_{O,2} \circ r_{O,\frac{\pi}{3}}$ dans un repère d'origine $O$.
$f$ est la similitude directe de centre $O$, rapport $2$, angle $\frac{\pi}{3}$.
Image de $A(1,0)$ : rotation $\to A_1(\frac12,\frac{\sqrt3}{2})$, homothétie $\to A'(1,\sqrt3)$.
Vérification analytique : $X = 2(x\frac12 - y\frac{\sqrt3}{2}) = x - \sqrt3 y$, $Y = 2(x\frac{\sqrt3}{2} + y\frac12) = \sqrt3 x + y$.
Pour $A(1,0)$ : $X=1, Y=\sqrt3$. Cohérent.
\end{exemplebox}

\begin{attention}[Ne pas confondre : Similitude directe vs indirecte]
\begin{itemize}
  \item \textbf{Directe} : composée rotation + homothétie (même centre). Conserve l'orientation (angles orientés). Matrice jacobienne de déterminant $k^2 > 0$.
  \item \textbf{Indirecte} : composée symétrie orthogonale + homothétie (ou rotation + homothétie + symétrie). Inverse l'orientation. Non au programme de Première C/D/E comme objet d'étude principal, mais bon à savoir.
\end{itemize}
Au Probatoire, « similitude » sans précision = \textbf{similitude directe}.
\end{attention}

\coursec{Synthèse et exercices d'entraînement}

\begin{aretenir}[Tableau récapitulatif des composées (Même centre)]
\begin{center}
\begin{tabularx}{\linewidth}{|l|X|X|}\hline
\textbf{Composée} & \textbf{Nature} & \textbf{Caractéristiques} \\ \hline
$r_{\Omega,\beta} \circ r_{\Omega,\alpha}$ & Rotation & Centre $\Omega$, Angle $\alpha+\beta$ \\ \hline
$h_{\Omega,k'} \circ h_{\Omega,k}$ & Homothétie & Centre $\Omega$, Rapport $kk'$ \\ \hline
$h_{\Omega,k} \circ t_{\vec{u}}$ ($k\neq1$) & Homothétie & Rapport $k$, Centre $\Omega'$ tel que $\overrightarrow{\Omega\Omega'} = \frac{k}{1-k}\vec{u}$ \\ \hline
$t_{\vec{u}} \circ h_{\Omega,k}$ ($k\neq1$) & Homothétie & Rapport $k$, Centre $\Omega''$ tel que $\overrightarrow{\Omega\Omega''} = \frac{1}{1-k}\vec{u}$ \\ \hline
$h_{\Omega,k} \circ r_{\Omega,\theta}$ & Similitude directe & Centre $\Omega$, Rapport $k$, Angle $\theta$ \\ \hline
\end{tabularx}
\end{center}
\end{aretenir}

\begin{exercice}[Entraînement : Reconnaissance analytique]
Dans un repère orthonormé, on définit les applications suivantes :
\begin{enumerate}
  \item $f_1 : \begin{cases} X = -x + 2 \\ Y = -y - 1 \end{cases}$
  \item $f_2 : \begin{cases} X = \frac12 x - \frac{\sqrt3}{2} y + 1 \\ Y = \frac{\sqrt3}{2} x + \frac12 y - 2 \end{cases}$
  \item $f_3 : \begin{cases} X = 3x - 2 \\ Y = 3y + 4 \end{cases}$
\end{enumerate}
Pour chacune, dire si c'est une transformation. Si oui, donner sa nature, ses éléments caractéristiques et son écriture analytique réciproque.
\end{exercice}

\begin{corrige}[Exercice 1]
\textbf{1.} $f_1$ : Matrice $\begin{pmatrix} -1 & 0 \\ 0 & -1 \end{pmatrix}$, déterminant $1 \neq 0$. C'est une transformation.
Équation point fixe : $x = -x+2 \Rightarrow x=1$ ; $y = -y-1 \Rightarrow y=-0,5$.
Centre $\Omega(1\,;-0,5)$. C'est une \textbf{rotation} de centre $\Omega$ et d'angle $\pi$ (ou homothétie de rapport $-1$, symétrie centrale).
Réciproque : $f_1^{-1} = f_1$ (involution). $\begin{cases} x = -X+2 \\ y = -Y-1 \end{cases}$.

\textbf{2.} $f_2$ : Matrice $\begin{pmatrix} 1/2 & -\sqrt3/2 \\ \sqrt3/2 & 1/2 \end{pmatrix}$, déterminant $1/4+3/4=1 \neq 0$. Transformation.
Forme $X = k(x\cos\theta - y\sin\theta) + c$, $Y = k(x\sin\theta + y\cos\theta) + c'$.
Ici $k=1$, $\cos\theta=1/2$, $\sin\theta=\sqrt3/2 \Rightarrow \theta = \pi/3$.
C'est une \textbf{rotation} de centre $\Omega$ (point fixe) et d'angle $\pi/3$.
Centre : $x = \frac12 x - \frac{\sqrt3}{2} y + 1$, $y = \frac{\sqrt3}{2} x + \frac12 y - 2$.
Résolution : $\frac12 x + \frac{\sqrt3}{2} y = 1$, $-\frac{\sqrt3}{2} x + \frac12 y = -2$.
Système : $x + \sqrt3 y = 2$, $-\sqrt3 x + y = -4$.
$y = \sqrt3 x - 4$. $x + \sqrt3(\sqrt3 x - 4) = 2 \Rightarrow 4x = 14 \Rightarrow x=3,5$. $y = 3,5\sqrt3 - 4$.
$\Omega(3,5\,; 3,5\sqrt3-4)$.
Réciproque : rotation de centre $\Omega$, angle $-\pi/3$.

\textbf{3.} $f_3$ : Forme $X = 3x - 2$, $Y = 3y + 4$. $k=3 \neq 0,1$. C'est une \textbf{homothétie} de rapport $3$.
Centre : $x = 3x-2 \Rightarrow x=1$ ; $y = 3y+4 \Rightarrow y=-2$. $\Omega(1\,;-2)$.
Réciproque : $h_{\Omega,1/3}$ : $\begin{cases} x = \frac13 X + \frac23 \\ y = \frac13 Y - \frac43 \end{cases}$.
\end{corrige}

\begin{exercice}[Composées : même centre]
Soient $r = r_{O,\frac{\pi}{2}}$, $h = h_{O,2}$, $t = t_{\vec{u}}$ avec $\vec{u}(1,1)$.
Déterminer la nature et les éléments caractéristiques de :
\begin{enumerate}
  \item $f_1 = r \circ h$
  \item $f_2 = h \circ t$
  \item $f_3 = r \circ t$ (rappel : composée rotation-translation = rotation de même angle, centre translaté)
\end{enumerate}
\end{exercice}

\begin{corrige}[Exercice 2]
\textbf{1.} $f_1 = r_{O,\pi/2} \circ h_{O,2} = h_{O,2} \circ r_{O,\pi/2}$ (commutent).
C'est une \textbf{similitude directe} de centre $O$, rapport $2$, angle $\frac{\pi}{2}$.
Analytique : $Z = 2 e^{i\pi/2} z = 2i z$. Soit $\begin{cases} X = -2y \\ Y = 2x \end{cases}$.

\textbf{2.} $f_2 = h_{O,2} \circ t_{(1,1)}$.
$t : (X,Y) = (x+1, y+1)$.
$h : (X',Y') = (2X, 2Y)$.
$f_2 : (X',Y') = (2x+2, 2y+2)$.
Rapport $k=2 \neq 1$ : \textbf{homothétie de rapport $2$}.
Centre $\Omega'$ : $x = 2x+2 \Rightarrow x=-2$ ; $y = 2y+2 \Rightarrow y=-2$.
$\Omega'(-2,-2)$. Vérification : $\overrightarrow{O\Omega'} = \frac{k}{1-k}\vec{u} = \frac{2}{-1}(1,1) = (-2,-2)$. \textbf{OK}.

\textbf{3.} $f_3 = r_{O,\pi/2} \circ t_{(1,1)}$.
On sait (hors programme détaillé mais résultat connu) que la composée d'une rotation et d'une translation est une rotation de même angle.
Centre $\Omega''$ : image de $O$ par $t$ puis $r$ ? Non.
Méthode analytique : $t : (X,Y)=(x+1,y+1)$. $r : (X',Y')=(-Y,X)$.
$f_3 : (X',Y') = (-(y+1), x+1) = (-y-1, x+1)$.
C'est une rotation d'angle $\pi/2$ (matrice $\begin{pmatrix}0&-1\\1&0\end{pmatrix}$).
Centre $\Omega''$ point fixe : $x = -y-1$, $y = x+1 \Rightarrow x = -(x+1)-1 \Rightarrow 2x = -2 \Rightarrow x=-1, y=0$.
$\Omega''(-1,0)$.
\end{corrige}

\begin{savaistu}[Le groupe des similitudes directes]
L'ensemble des similitudes directes du plan (rotations, homothéties $k>0$, translations, identité) muni de la composition forme un \textbf{groupe} : le groupe des similitudes directes $Sim^+(\mathbb{R}^2)$. C'est un groupe de Lie de dimension 4 (2 pour le centre, 1 pour l'angle, 1 pour le rapport). Les isométries directes (rotations, translations) en sont le sous-groupe $k=1$. Ces structures sont au cœur de la géométrie moderne et de la vision par ordinateur (reconnaissance de formes, réalité augmentée).
\end{savaistu}
