% Angles orientés et arcs capables — Mathématiques 1ère E — Cours signé OnBuch+
...
\item \textbf{Identifier la congruence} : $[2\pi]$ impose le signe de la mesure, donc sélectionne un seul arc sur un cercle ; $[\pi]$ ignore le signe, donc réunit les deux arcs de \emph{chaque} cercle concerné — ce qui donne deux cercles complets (sauf si $x=\frac{\pi}{2}$ où ils fusionnent).
\item \textbf{Conclure avec précision} : deux arcs symétriques sur deux cercles (angle géométrique), un seul arc sur un cercle (vecteurs, $[2\pi]$), deux cercles entiers symétriques (droites, $[\pi]$), ou un cercle (droites, $[\pi]$, $x=\frac{\pi}{2}$).
...
\begin{itemize}
\item \textbf{Erreur n°1} : conclure « le lieu est un cercle » alors que la condition est donnée \emph{modulo $2\pi$}. Avec des vecteurs et une congruence $[2\pi]$, le lieu est \textbf{un seul arc}, pas le cercle entier. Le cercle complet n'est correct que pour un angle de \emph{droites} (modulo $\pi$) \textbf{lorsque $x=\frac{\pi}{2}$} ; dans le cas général $x \neq \frac{\pi}{2}$, le lieu pour les droites est \textbf{deux cercles} symétriques.
...
\end{scope}
% --- Figure 3 : droites [π] : deux cercles complets (cas général x≠π/2) ---
\begin{scope}[shift={(5.2,0)}]
\coordinate (A3) at (-1,0); \coordinate (B3) at (1,0);
% Cercle 1
\draw[poporange,very thick] (0,0) circle(1);
% Cercle 2 (symétrique)
\draw[poporange,very thick] (0,0) circle(1); % Wait, symmetric about AB (x-axis) means centers at (0, h) and (0, -h).
% Let's draw two circles centered at (0, 0.5) and (0, -0.5) radius sqrt(1+0.25)? No, simpler: radius 1, centers (0,0) is diameter.
% For x=pi/3, circles have centers on perpendicular bisector (y-axis). Radius = 1/sin(pi/3) = 2/sqrt(3) ~ 1.15. Center height = cot(pi/3) = 1/sqrt(3) ~ 0.577.
% Let's just draw two circles intersecting at A,B symmetric about x-axis.
...
\node[poporange,align=center] at (0,-1.7) {\footnotesize droites $[\pi]$ ($x\neq\frac{\pi}{2}$) :\\ \footnotesize deux cercles symétriques};
\end{scope}
\end{tikzpicture}
\end{center}
\legende{Le même segment $[AB]$, trois conditions angulaires, trois lieux différents (dans tous les cas, $A$ et $B$ sont exclus). Pour les droites modulo $\pi$, le cas général ($x\neq\frac{\pi}{2}$) donne deux cercles ; le cas $x=\frac{\pi}{2}$ donne le cercle de diamètre $[AB]$ (figure 2 de l'exemple ci-dessus).}
\end{popfigure}
...
\hline
$\operatorname{mes}((MA),(MB)) = x \;[\pi]$ & Réunion de deux cercles symétriques par rapport à $(AB)$, passant par $A$ et $B$, privés de $A$ et $B$ & $x = \frac{\pi}{2}$ : cercle de diamètre $[AB]$ privé de $A$ et $B$ (les deux cercles se confondent) \\
\hline
\end{tabularx}
\end{center}
\end{aretenir}
...
\item $\mathcal{E}_3$ : modulo $\pi$, les mesures $\frac{\pi}{3}$ et $\frac{\pi}{3} - \pi = -\frac{2\pi}{3}$ sont confondues (elles diffèrent de $\pi$). Sur \emph{chacun} des deux cercles de $\mathcal{E}_1$, les deux arcs se réunissent. $\mathcal{E}_3$ est donc la \textbf{réunion des deux cercles entiers} passant par $A$ et $B$ (ceux qui portent les arcs de $\mathcal{E}_1$), privés de $A$ et $B$.
\end{enumerate}
On observe les inclusions : $\mathcal{E}_2 \subset \mathcal{E}_1$ et $\mathcal{E}_2 \subset \mathcal{E}_3$. $\mathcal{E}_1$ est la réunion de deux arcs (un sur chaque cercle), $\mathcal{E}_3$ est la réunion des deux cercles complets. Cet exercice montre qu'une lecture précise de la congruence est la clé de toute la question.
\end{exoresolu}
...
\item \textbf{Lieu orienté modulo $\pi$} : l'ensemble des points $M$ tels que $\mathrm{mes}(\overrightarrow{MA},\overrightarrow{MB}) = x + k\pi$ ($k\in\mathbb{Z}$) est la réunion des \emph{deux cercles entiers} (privés de $A$ et $B$) dont les arcs capables correspondent aux angles non orientés $x$ et $\pi-x$. (Pour $x=\frac{\pi}{2}$, les deux cercles coïncident en le cercle de diamètre $[AB]$.)
...
\end{enumerate}
Le lieu $\mathcal{L}$ des positions possibles (angle non orienté $60^{\circ}$) est la réunion des deux arcs d'extrémités $A$ et $B$ situés de part et d'autre de $(AB)$ (les arcs « mineurs » des deux cercles), \textbf{privée des points $A$ et $B$} : en effet, si $M = A$ ou $M = B$, l'angle $\widehat{AMB}$ n'est pas défini.
...
\begin{itemize}
\item \textbf{Condition $\mathrm{mes}(\overrightarrow{MA},\overrightarrow{MB}) = \frac{\pi}{3} + 2k\pi$.} Cette condition impose non seulement la \emph{valeur} de l'angle ($60^{\circ}$) mais aussi son \emph{sens} (direct). Sur le cercle supérieur (centre $O$), l'arc mineur (contenant $M_1$) donne un angle orienté $+\frac{\pi}{3}$ ; l'arc majeur donne $-\frac{2\pi}{3} \equiv \frac{4\pi}{3}\ [2\pi]$. Sur le cercle inférieur (centre $O'$), l'arc mineur donne $-\frac{\pi}{3}$ et l'arc majeur donne $+\frac{2\pi}{3}$. \textbf{Un seul arc} (ici l'arc mineur du cercle supérieur, contenant $M_1$) satisfait la condition modulo $2\pi$. La moitié du lieu non orienté disparaît.
\item \textbf{Condition $\mathrm{mes}(\overrightarrow{MA},\overrightarrow{MB}) = \frac{\pi}{3} + k\pi$.} Travailler modulo $\pi$ revient à identifier les angles de \emph{droites} : on ne distingue plus $\frac{\pi}{3}$ de $\frac{\pi}{3}+\pi = \frac{4\pi}{3}$. Le lieu est l'ensemble des points $M$ tels que l'angle non orienté entre les droites $(MA)$ et $(MB)$ vaut $\frac{\pi}{3}$ \emph{ou} $\pi-\frac{\pi}{3}=\frac{2\pi}{3}$. Or, sur chacun des deux cercles, les arcs mineurs donnent l'angle $60^{\circ}$ et les arcs majeurs donnent l'angle $120^{\circ}$. Le lieu est donc la réunion des \textbf{deux cercles entiers}, privés des points $A$ et $B$. Ce n'est \emph{pas} le même ensemble que le lieu non orienté de la question 2 (qui ne contenait que les arcs mineurs).
\end{itemize}
...
\item \textbf{Modulo $\pi$ $\neq$ non orienté} : le lieu $\mathrm{mes}(\overrightarrow{MA},\overrightarrow{MB}) = x + k\pi$ est l'union des deux cercles entiers (arcs pour $x$ et arcs pour $\pi-x$), alors que le lieu non orienté $\mathrm{mes}(\widehat{AMB})=x$ ne contient que les arcs pour $x$.
\end{itemize}
\end{attention}
...
\item L'orientation du plan \emph{affine} le lieu : modulo $2\pi$, un seul arc subsiste ; modulo $\pi$, on obtient les deux cercles entiers (arcs pour $x$ et pour $\pi-x$). Savoir passer d'un langage à l'autre est indispensable au Probatoire.
\end{itemize}
