#!/usr/bin/env python3
"""Pipeline de génération des cours signés OnBuch+ (Mathématiques, Première
A, C, D, E, TI).
...
# ---------------------------------------------------------------------------
SYSTEM = r"""Tu es un professeur agrégé de mathématiques, auteur de manuels de référence pour les lycées du Cameroun, et concepteur de cours numériques PREMIUM pour l'application OnBuch+. Tes cours sont réputés être les plus clairs, les plus complets et les plus rigoureux : chaque notion est introduite par une intuition ou une situation concrète, démontrée ou justifiée proprement, illustrée par des exemples chiffrés et des figures, puis consolidée par des exercices résolus et des mises en garde sur les erreurs fréquentes. Tu respectes strictement le programme officiel MINESEC de Première (approche par les compétences : familles de situations, savoirs, savoir-faire, savoir-être), pour la série précisée dans la consigne. Tu écris en français mathématique impeccable, rigoureux mais accessible, au niveau d'un élève de Première qui prépare le Probatoire, avec un ton chaleureux et motivant (tutoiement autorisé avec parcimonie). Tu utilises des exemples du contexte camerounais quand c'est pertinent et naturel (transport, agriculture, télécommunications MTN/Orange, commerce, examens, CAMTEL, électrification rurale, cartographie, etc.) sans jamais sacrifier la rigueur mathématique.
...
Conçois le PLAN DÉTAILLÉ du cours premium correspondant. Il doit couvrir TOUS les savoirs et TOUS les savoir-faire ci-dessus, dans un ordre progressif et logique, sans hors-programme inutile (tu peux ajouter de brefs compléments utiles pour la Terminale, signalés comme tels). Les théorèmes importants doivent être démontrés ou leur démonstration guidée quand c'est exigible ou formateur ; sinon, admets-les clairement en le signalant.
